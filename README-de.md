...
Übersetzer für Markdown-Dateien mit **OpenAI**, **Mistral AI**, **Claude (Anthropic)**, **Google Gemini** und **Grok (xAI)** – über API, über das Kontingent eines ChatGPT-Abonnements (Codex) oder Grok ohne nutzungsabhängige Abrechnung oder über **OpenCode**, den Open-Source-Agenten, mit einem Anbieter Ihrer Wahl: lokales Modell (Ollama), kostenlos, Abonnement (GitHub Copilot…) oder Schlüssel.
...
- **Multi-Provider**: 5 APIs (OpenAI, Mistral, Claude, Gemini, Grok) + 2 CLIs über ein Abonnement ohne nutzungsabhängige Abrechnung – Codex (ChatGPT) und Grok – + OpenCode (Open Source, MIT) für jeden in OpenCode konfigurierten Anbieter, einschließlich eines lokalen Modells
...
Der Befehl `aipmt` ist anschließend überall verfügbar. Falls sich das Python-Skriptverzeichnis
nicht in Ihrem `PATH` befindet, bewirkt `python -m aipmt` genau das Gleiche.
...
Das geklonte Repository wird weiterhin für die Entwicklung benötigt: Dort befinden sich die Tests,
...
`requirements.txt` ist eine **vollständig festgeschriebene Lock-Datei** und bildet die
getestete Umgebung exakt ab. Die in `pyproject.toml` veröffentlichten Grenzen sind
bewusst weiter gefasst: Sie erzwingen keine Vorgaben für Ihre anderen Pakete.
...
Jede Stelle ergänzt nur, was die vorherige offengelassen hat.

|     | Wo                                            | Wofür                                  |
| --- | --------------------------------------------- | -------------------------------------- |
| 1   | Umgebungsvariablen                            | CI, Container, punktuelle Abweichungen |
| 2   | `.env` im aktuellen Verzeichnis (oder einem übergeordneten Verzeichnis) | ein projektspezifischer Schlüssel      |
...
(andernfalls wird sie entsprechend der Spezifikation ignoriert), und `%APPDATA%`
unter Windows.

Die zweite Möglichkeit bleibt nützlich, wenn ein Repository einen eigenen Schlüssel besitzt: Eine Datei `.env` in dessen Stammverzeichnis
...
listet die drei Speicherorte mit ihrem jeweiligen vollständigen Pfad auf.

`GEMINI_API_KEY` wird als Alternative zu `GOOGLE_API_KEY` akzeptiert (AI-Studio-Konvention).
Optionale Variablen: `XAI_BASE_URL` (xAI-Endpunkt, Standard
...
`OPENCODE_TIMEOUT` (siehe Abschnitt OpenCode) sowie `OPENROUTER_BASE_URL` /
`OPENROUTER_TIMEOUT` / `OPENROUTER_PREFLIGHT_TIMEOUT` (siehe Abschnitt
OpenRouter). Für
`regen_translations.sh`: `REGEN_PROVIDER` (Standard `codex`, über Abonnement),
`REGEN_MODEL`, `REGEN_ALLOW_PAID_API` (zwingend erforderliche Abweichung für eine
kostenpflichtige API) und `REGEN_JOB_TIMEOUT` (Obergrenze pro Job, standardmäßig 600 s, bei Codex 1.800 s).
...
einzige von OpenAI dokumentierte Weg für diese Nutzung – die Tokens von `~/.codex/auth.json`
authentifizieren keine Aufrufe an die API Platform und werden von diesem Skript
ohnehin nie gelesen.
...
Die ausführbare Datei wird in dieser Reihenfolge gesucht: in der Variable `CODEX_BIN`, im `PATH`
...
Benutzern für einen optionalen Provider aufgezwungen würde.

**Wissenswertes:**

- **Es wird kein API-Schlüssel verwendet.** `OPENAI_API_KEY` und `CODEX_API_KEY` werden
  aus der Umgebung des Unterprozesses entfernt. Dadurch ist gewährleistet, dass ein
  in `.env` vorhandener Schlüssel niemals dazu führt, dass die Übersetzung auf
  eine nutzungsabhängige Abrechnung umgestellt wird.
...
- **In CI abgelehnt** (`CI` oder `GITHUB_ACTIONS` definiert): Das Abonnement
  authentifiziert sich über eine persönliche Sitzungsdatei. Diese auf einen gemeinsam
  genutzten Runner zu übertragen, bedeutet, dort eine Identität zu hinterlegen, die von
  allem dort Ausgeführten wiederverwendet werden kann. Verwenden Sie für diesen Weg einen API-Schlüssel.
- Umgebungsvariablen: `CODEX_BIN` (expliziter Pfad zur ausführbaren Datei) und
  `CODEX_TIMEOUT` (Sekunden pro Segment, Standard `600`).

### Über das eigene Grok-Abonnement übersetzen (`--use_grok_cli`)

Dasselbe Prinzip wie bei `--use_codex`, mit der offiziellen CLI **Grok Build**: Die
Übersetzung wird auf das Grok-Abonnement (SuperGrok / X Premium+) angerechnet, statt
...
**Isolation – vor der Verwendung lesen.** Dieser Provider ist strukturell **schwächer**
als `--use_codex`, und das ist eine bewusste Entscheidung:

- Codex läuft in `--sandbox read-only`, einer vom System erzwungenen Grenze.
- Die Sandbox von Grok **kann auf vielen aktuellen Linux-Systemen nicht angewendet
  werden**: AppArmor blockiert seit Ubuntu 24.04 nicht privilegierte User Namespaces,
  und die Sperrliste für Container-Runtime-Sockets schlägt fehl, wenn
  `/run/podman` auf `0700` gesetzt ist. Ein **integriertes** Profil, das nicht
  angewendet werden kann, startet jedoch **ohne Isolation und ohne Hinweis**.
- Das Skript fordert daher standardmäßig kein Profil an und **fällt niemals
  unbemerkt zurück**: Es zeigt eine Warnung an. Die Isolation beruht auf den
  `--deny`-Regeln der CLI (einschließlich des Catch-all `*`), der einzigen als
  _fail-closed_ gemessenen Schicht – eine unbekannte Regel verhindert den Start,
  statt den Schutz ohne Hinweis zu entfernen.
- Um die Betriebssystem-Sandbox **zwingend vorauszusetzen**: `GROK_TRANSLATE_SANDBOX=read-only`. Der
  Start schlägt fehl, wenn das System sie nicht bereitstellen kann, was dem
  gewünschten Verhalten entspricht.

**Kontingent**: Der Grok-Pool ist **wöchentlich und wird gemeinsam** mit Chat, Imagine und
Voice genutzt; außerdem kann er mit keinem Befehl abgefragt werden. Eine Stapelverarbeitung
kann daher Ihre Nutzung für Unterhaltungen beeinträchtigen, ohne dass darauf hingewiesen
wird – deshalb ist die Parallelität auf 2 begrenzt und `regen_translations.sh` enthält eine Warnung.

Weitere Variablen: `GROK_BIN` (Pfad zur ausführbaren Datei), `GROK_TIMEOUT` (Standard 900 s).
...
```
### Mit OpenCode über einen Anbieter Ihrer Wahl übersetzen (`--use_opencode`)
...
nicht interaktiven Modus und beschränkt den Aufruf auf einen einzigen
Hin-und-zurück-Durchlauf ohne jegliche Tools.

Zwei dieser Wege wurden hier durchgängig gemessen: das **Zen-Gateway** und
**Ollama** lokal. Die anderen von OpenCode angekündigten Optionen (GitHub Copilot,
LM Studio, llama.cpp) sollten konstruktionsbedingt funktionieren, da der Provider
nur mit OpenCode kommuniziert — sie wurden jedoch nicht getestet, und dieses
README beschreibt ausschließlich das, was überprüft wurde.
...
Anbieter, und es wird nicht an Ihrer Stelle ein Standardwert gewählt: Der eigene
Fallback wäre ein kostenloses Modell, dessen Interaktionen für das Training
...
**Beschränkung — was das Skript bei jedem Aufruf tut:**
...
- Eine Inline-Konfiguration (`OPENCODE_CONFIG_CONTENT`), die Vorrang vor Ihrer eigenen hat,
  ...
  ausführen — den Messungen zufolge versucht es dies nicht einmal. Die
  Sitzungsfreigabe ist deaktiviert, `--pure` schließt externe Plugins aus,
  niemals `--auto`.
- Der Aufruf läuft in einem **leeren Wegwerfverzeichnis** mit den Schaltern
  ...
  bei einer Messung wurde eine in einer `AGENTS.md` hinterlegte Anweisung,
  „jede Antwort mit BANANA beenden“, auf die Übersetzung angewendet. Die globalen
  Regeln von `~/.config/opencode/AGENTS.md` werden hingegen weiterhin angewendet: OpenCode
  erlaubt nicht, sie auszuschließen.
- Der Ausgabevertrag verlangt gleichzeitig: Rückgabecode 0, kein
  `error`-Ereignis, keinen Tool-Aufruf, einen letzten in `stop`
  abgeschlossenen Schritt, einen nicht leeren Text und den tatsächlich geladenen
  Agenten — ein unbekannter `--agent` lässt OpenCode nicht fehlschlagen,
  sondern es **fällt stillschweigend** auf den Coding-Agenten mit aktiven Tools
...
- **Kein aipmt-Schlüssel wird an den Unterprozess weitergegeben** (dieselbe
  ...
  Ausnahme: `OPENCODE_API_KEY`, der Schlüssel von OpenCode selbst (Zen, Go). Die
  Anbieter werden in OpenCode (`opencode auth login`, `opencode.json`) konfiguriert,
  nicht in der `.env` von aipmt.

**Wissenswertes:**

- **Die kostenlosen Zen-Modelle sind „Stealth“- oder Contributor-Modelle**,
  wechseln häufig, haben nicht dokumentierte Beschränkungen, und ihre
  Interaktionen können für das Training verwendet werden: ideal für öffentliche
  Dokumentation, für private Inhalte jedoch zu vermeiden. Gemessen:
  `opencode/mimo-v2.5-free` übersetzt dieses README in einem Durchlauf; `opencode/big-pickle` ist
  langsamer, und zwei gleichzeitige Anfragen blieben dort unbeantwortet.
- **Ein lokales Modell muss mindestens 16 k Kontext bieten** — die Segmente sind
  bis zu 16.000 Zeichen lang —, während Ollama standardmäßig oft 4.096
  konfiguriert. Mit Ollama: ein `Modelfile` mit `PARAMETER num_ctx 32768`, dann
  `ollama create`. Die Qualität hängt vom Modell ab: Ein 7B-Modell kehrte in
  einer Testdatei eine Liste um und beschädigte den Abschluss eines Codeblocks,
  während ein Gateway-Modell alles beibehielt.
- `--eco` hat keine Wirkung (das Modell ist das aus `--model`);
  `--reasoning_effort` wird unverändert als `--variant` von OpenCode übergeben und
  sollte nur angefordert werden, wenn das Modell es kennt.
- Die Sitzungen werden von OpenCode wie jede andere OpenCode-Sitzung in seiner
  Datenbank (`~/.local/share/opencode/`) protokolliert.
- Umgebungsvariablen: `OPENCODE_BIN` (expliziter Pfad zur Binärdatei, andernfalls
  `PATH` und danach `~/.opencode/bin/opencode`) und `OPENCODE_TIMEOUT` (Sekunden pro
  Segment, Standardwert `600`). `OPENCODE_CONFIG` wird berücksichtigt,
  wenn Sie es exportieren.

**Gemessenes Beispiel: ein lokales Modell über Ollama** (RTX 3060 mit 12 GB, 62 GB RAM, Ollama 0.33.3)
...
Dann der Anbieter in `~/.config/opencode/opencode.json`:
...
standardmäßig das Reasoning, und ein Modelfile kann es nicht deaktivieren.
...
Ergebnisse für einen echten Blogartikel mit 589 Zeilen (140 Links, 21
...
| `ollama/qwen3.5-9b-32k` (lokal)          | 8 min 18 s  | Links, URLs, Tabellen und Tags identisch                   | eine erfundene Zitatzeile, etwas Fettdruck und Inline-Code hinzugefügt, ein Segment erneut verarbeitet |

Diese beiden lokalen Modelle wurden inzwischen **ausgeschlossen**: Eine einzige
freie Abweichung pro Artikel genügt, um ein Modell für veröffentlichte
Übersetzungen zu disqualifizieren. Fünf weitere wurden aus denselben Gründen
oder wegen Zeitüberschreitung ausgeschlossen (`gemma4:26b-a4b`,
...
Artikel französische Passagen stehen; siehe die Tabelle der empfohlenen Modelle.

Während der lokalen Übersetzung: GPU bei 98 % und 170 W, 10 GB belegter VRAM
(Modell und Cache für 32 k Tokens, nichts in den RAM ausgelagert), 7,5 GB RAM
für den Ollama-Server. Ein Modell mit 9 bis 12 Milliarden Parametern hält die
Struktur ein, erlaubt sich aber eine freie Abweichung pro Artikel, während das
Gateway-Modell keine einzige vorgenommen hat: vor der Veröffentlichung
Korrektur lesen oder auf Entwürfe beschränken.

### Über OpenRouter übersetzen (`--use_openrouter`)

OpenRouter ist ein **Router** vor mehr als 400 von Drittanbietern gehosteten
Modellen, die nutzungsabhängig über ein gemeinsames Guthaben abgerechnet werden.
Mit einem einzigen Schlüssel bietet er Zugriff auf Modelle, die keiner der
anderen Provider bereitstellt, insbesondere auf offene chinesische Modelle.
...
- **Dasselbe Modell wird von Dutzenden Hostern mit unterschiedlichen Limits
  bereitgestellt.** Für `z-ai/glm-5.3-flash` gibt es 23 Hoster, darunter einen mit
  einem Ausgabelimit von 2.048 Tokens: Ohne Vorkehrungen wurde eine lange
  Übersetzung bei einer von 23 Anfragen abgeschnitten, zufällig durch das Routing
  und ohne jedes Signal. Ein Preflight liest `/api/v1/models/{modèle}/endpoints`, schließt Hoster mit
  weniger als 8.000 Ausgabetokens oder mit beeinträchtigtem Status aus und
...
- **Reasoning wird zum Ausgabetarif abgerechnet.** Dieselbe Anfrage an
  ...
  bei Modellen, die dies zulassen, standardmäßig deaktiviert. Modelle, die es
  ...
  **niedrigsten von ihnen als akzeptiert angegebenen Aufwand** und nicht ihre
  Standardeinstellung: Die von `z-ai/glm-5.3-flash` ist `max`, und damit
  waren die 32.768 Ausgabetokens bereits vor dem Ende der Übersetzung
  ausgeschöpft. Eine Erhöhung des Limits hätte daran nichts geändert, da der
  Aufwand einen Prozentsatz davon beansprucht. `--reasoning_effort` behält Vorrang,
  und `none` bei einem Modell, das Reasoning erzwingt, wird gemeldet,
  anstatt umgangen zu werden.

Der Preflight arbeitet **fail-closed** und zeigt an, was er ausgewählt hat:
...
Ein nicht im Katalog vorhandener Slug, ein nicht erreichbarer Katalog oder das
Fehlen eines Hosters, der das Limit einhalten kann, beendet den Befehl vor jeder
Abrechnung.

Weitere Punkte:

- Das Kontextfenster stammt aus dem Katalog und nicht aus einer Konstanten: Die
  Segmentierung passt sich tatsächlich daran an, auch bei Modellen mit 4.095
  Tokens.
- `--eco` hat keine Wirkung (das Modell ist das aus `--model`).
- `finish_reason=length` mit leerer Ausgabe ist keine Abschneidung, sondern ein durch
  das Reasoning aufgebrauchtes Budget; die Meldung weist darauf hin, weil beide
  Fälle entgegengesetzte Maßnahmen erfordern.
...
| `--use_opencode`         | OpenCode (Open Source) über den in OpenCode konfigurierten Anbieter verwenden; erfordert `--model provider/modèle`      |
| `--force`                | Erneute Übersetzung erzwingen                                                                                 |
| `--keep_filename`        | Ursprünglichen Dateinamen beibehalten                                                                          |
| `--news`                 | Nachrichtenmodus: schützt englische Zitate und verarbeitet Flaggen nach Sprache                               |
...
> **Die sieben Provider-Flags schließen sich gegenseitig aus.** Zuvor wurde die
> Kombination zweier Flags stillschweigend akzeptiert und auf den zuerst
> geprüften Provider aufgelöst: Eine Übersetzung, die über ein
> Abonnementkontingent angefordert wurde (`--use_codex`, `--use_grok_cli`),
> konnte so ohne jede Warnung nutzungsabhängig abgerechnet werden.
...
Stellen platzieren und ihn entweder als einfaches Textformat
(abwärtskompatibel) oder als von einem Markdown-Plugin verarbeitbares
`marker`-Format ausgeben.

**Position** (`--note_position`):

- `bottom` (Standardwert): Hinweis wie bisher am Dateiende.
- `top`: Hinweis wird **nach dem YAML-Frontmatter** eingefügt
  (kompatibel mit Astro Content Collections, gray-matter usw.).
...
- `marker`: unsichtbare Markdown-Linkreferenzdefinition
  ...
  GitHub/GitLab lesbar und beim Build durch ein serverseitiges remark-Plugin in
  Astro auswertbar, um ein stilisiertes Banner zu erzeugen (siehe Blog
  jls42.org).
...
| Provider   | Qualität (Standardwert)                  | Sparsam (`--eco`)  |
| ---------- | ---------------------------------------- | ------------------------- |
...
Ein Modell, das einen Absatz gut übersetzt, erhält nicht unbedingt die Struktur
eines ganzen Dokuments. Diese Messwerte stammen aus **tatsächlich
durchgeführten Übersetzungen** mit dem weiter oben gezeigten Befehl, anhand von
drei Dokumentsätzen und vierzehn Zielsprachen: en, es, de, it, pt, nl, pl, sv,
ro, ja, ko, zh, ar, hi.

Zwei Spalten, die nicht dasselbe aussagen. **Erstellt** zählt die erfolgreich
abgeschlossenen Übersetzungen – die Schutzmechanismen des Skripts gegen
unbemerkte Fehler lassen die Datei passieren. **Ohne Abweichung** zählt
diejenigen, deren Struktur mit der Quelle identisch ist: gleiche Abschnitte,
gleiche Links, gleiche URLs, gleiche Blöcke und Inline-Codes, gleiche Tabellen,
gleiche Zitate, gleiche Flags.

### Umfangreicher Blogartikel, Modus `--news`

589 Zeilen, 140 Links, 21 Abschnitte, 3 geschützte englische Zitate. Dies ist das
anspruchsvollste der drei Dokumente: Der Modus `--news` ergänzt die
Markdown-Struktur um Vorgaben für Flags und Zitate.
...
Zwei Durchläufe wurden **wegen fehlenden Guthabens abgebrochen**, was sich in
ihren Nennern widerspiegelt: `qwen3.5-27b` stoppte nach neun Sprachen,
`kimi-k2.6` nach vier – Letzteres nach einer Zeitüberschreitung von vierzig
Minuten und zwei Ablehnungen, bei fast 0,33 $ pro Sprache.

Ein methodischer Vorbehalt zu den OpenRouter-Zeilen: Sie wurden mit den
**Standardeinstellungen des Routers** gemessen, bevor `--use_openrouter`
existierte. `z-ai/glm-5.2` wurde inzwischen mit dem mitgelieferten Provider und
deaktiviertem Reasoning erneut gemessen und erzielt exakt dasselbe Ergebnis von
14/14. `z-ai/glm-5.3-flash` scheiterte zweimal, weil das Ausgabebudget mit den
Router-Standardeinstellungen ausgeschöpft war; der Provider fordert von diesen
Modellen nun den niedrigsten von ihnen akzeptierten Aufwand an, und die
Gegenprobe mit den zuvor fehlerhaften Sprachen gelingt.

### README dieses Projekts, Standard-Markdown

508 Zeilen, 219 Inline-Codes, 40 Blockbegrenzungen, 45 Tabellenzeilen. Hier gibt
es keinen Modus `--news`: Die Schwierigkeit entsteht durch die hohe
Code-Dichte.
...
Dokumente sind **einfacher** als die beiden vorherigen, was die Tabelle
verdeutlicht.
...
- **Drei Modelle haben bei den beiden umfangreichen Dokumenten nie Informationen
  verloren**: `gemini-3.7-flash`, `gpt-5.6-sol` über das ChatGPT-Abonnement und
  `z-ai/glm-5.2` über OpenRouter. Ihre einzigen Abweichungen im Standardmodus
  bestehen aus einem Paar `**`, das in einer oder zwei Sprachen nicht
  übernommen wurde, niemals aus einer URL, einem Codeblock oder einem Zitat.
- **Der entscheidende Faktor ist die Dichte des Dokuments, nicht der Modus
  `--news`.** Grok über das Abonnement scheitert beim Blogartikel 13-mal
  von 14 Versuchen und bewältigt 14 von 16 öffentlichen README-Dateien: Ursache
  des Scheiterns ist ein Abbruch bei einem langen Segment, wie eine Gegenprobe
  bestätigte – die isolierte Passage wird korrekt übersetzt.
- **Nichtlateinische Schriften sind nicht die erwartete Trennlinie.**
  `gpt-oss` belässt Passagen auf Französisch im Arabischen, Japanischen,
  Polnischen **und Rumänischen**; Mistral und MiMo verlieren Inline-Codes nur bei
  nichtlateinischen Schriften.
- **Das Deaktivieren des Reasonings beeinträchtigt die Qualität nicht.**
  `z-ai/glm-5.2` bewältigt unter beiden Bedingungen vierzehn Sprachen ohne eine
  einzige Abweichung – zunächst mit dem standardmäßig vom Router aktivierten
  Reasoning, dann durch `--use_openrouter` deaktiviert – und verursacht dabei
  achtzehnmal weniger berechnete Ausgabe-Tokens. Diese Messung rechtfertigt die
  Standardeinstellung des Providers.
- **Ein langsames Modell ist kein zuverlässiges Modell.** `deepseek-v4-flash-0731`
  benötigt 37 Minuten pro Sprache für 4 von 14 Übersetzungen,
  `qwen3.8-flash` 26 Minuten für ein nahezu perfektes Ergebnis und Gemini
  1 Minute 18 Sekunden für ein fehlerfreies Ergebnis.
...
- **Sie ist keine umfassende Rangliste.** Allein OpenRouter bietet mehr als
  ...
  Modells sagt nichts über seine Qualität aus, sondern lediglich, dass es nicht
  getestet wurde.
- **Diese Messwerte haben ein Datum**: den 4. und 5. September 2026. Modelle
  verändern sich unter demselben Namen, Anbieter passen Quantisierungen und
  Obergrenzen an, und jede Woche erscheinen neue Modelle.
- **Die Laufzeiten ergeben keine Rangfolge.** Je nach Testlauf wurden 3 bis 6
  Übersetzungen parallel ausgeführt, und der Durchsatz eines Anbieters schwankt
  im Tagesverlauf. Sie vermitteln eine Größenordnung, keinen Vergleich.
- **Ein Ergebnis hängt ebenso sehr vom Dokument wie vom Modell ab.** Dasselbe
  Modell bewältigt bei einem Artikel vierzehn Sprachen und bei diesem README
  neun. Ihre Dateien sind nicht unsere.
- **Der richtige Ansatz bleibt, selbst zu messen**: Übersetzen Sie eines Ihrer
  Dokumente in Ihre Zielsprachen und vergleichen Sie anschließend die Struktur
  – die Anzahl der Abschnitte, Links, unterschiedlichen URLs, Codeblöcke,
  Inline-Codes und Tabellenzeilen. Genau das leistet das oben beschriebene
  Protokoll, und es passt in eine Schleife über `aipmt`.
