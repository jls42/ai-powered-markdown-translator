...
Übersetzer für Markdown-Dateien mit **OpenAI**, **Mistral AI**, **Claude (Anthropic)** und **Google Gemini**.
...
- **Multi-Provider**: Unterstützung für 4 APIs (OpenAI, Mistral, Claude, Gemini) sowie die Codex-CLI mit ChatGPT-Abonnement
...
- **Codeerhaltung**: Codeblöcke UND Inline-Code (`` `...` ``) werden bewahrt
...
Der Befehl `aipmt` ist anschließend überall verfügbar. Wenn das Python-Skriptverzeichnis nicht in deinem `PATH` enthalten ist, erledigt `python -m aipmt` genau dasselbe. Python 3.10 oder neuer.
...
`requirements.txt` ist ein **vollständig festgepinntes Lockfile**, das die getestete Umgebung exakt abbildet. Die in `pyproject.toml` veröffentlichten Grenzen sind absichtlich weiter gefasst: Sie machen keine Vorgaben für deine anderen Pakete.

### Qualitätssicherung (optional, aber empfohlen)

Das Projekt verwendet [`pre-commit`](https://pre-commit.com), um das Committen schlecht formatierten, anfälligen oder Geheimnisse enthaltenden Codes zu verhindern. Installation:
...
Aktive Hooks: ruff (Linting+Formatierung), shellcheck (bash), prettier (markdown/yaml/json), Lizard (Komplexität), detect-secrets (API-Schlüssel), mypy (schrittweise Typisierung), Opengrep (SAST), pip-audit (CVE-Abhängigkeiten), unittest. Siehe Abschnitt _Quality / pre-commit_ in `CLAUDE.md` für weitere Details.

## Konfiguration

Erstelle eine Datei `.env` **in dem Verzeichnis, aus dem du den Befehl startest** (sie wird dort und anschließend in den übergeordneten Verzeichnissen gesucht) oder definiere die Umgebungsvariablen:
...
- **Es wird kein API-Schlüssel verwendet.** `OPENAI_API_KEY` und `CODEX_API_KEY` werden aus der Umgebung des Unterprozesses entfernt. Dadurch wird garantiert, dass ein in `.env` vorhandener Schlüssel die Übersetzung niemals auf nutzungsbasierte Abrechnung umstellt.
...
- **Langsamer** als ein API-Aufruf: Für ein vollständiges README etwa 45 s einplanen, gegenüber wenigen Sekunden direkt.
- **In CI abgelehnt** (`CI` oder `GITHUB_ACTIONS` definiert): Die Authentifizierung per Abonnement ist nicht für einen gemeinsam genutzten Runner vorgesehen, und OpenAI rät von diesem Workflow in öffentlichen Repositories ab. Verwende auf diesem Weg einen API-Schlüssel.
...
Dasselbe Prinzip wie bei `--use_codex`, mit der offiziellen **Grok Build**-CLI: Die Übersetzung wird vom Grok-Abonnement (SuperGrok / X Premium+) abgezogen, statt tokenbasiert abgerechnet zu werden.
...
- Die Grok-Sandbox kann auf vielen aktuellen Linux-Systemen nicht angewendet werden: AppArmor blockiert seit Ubuntu 24.04 nicht privilegierte User-Namespaces, und die Denylist für Container-Runtime-Sockets schlägt fehl, wenn `/run/podman` auf `0700` steht. Ein **integriertes** Profil, das nicht angewendet werden kann, startet jedoch **still und ohne Einschränkung**.
- Das Skript fordert daher standardmäßig kein Profil an und fällt niemals **still** zurück: Es zeigt eine Warnung an. Die Einschränkung beruht auf den `--deny`-Regeln der CLI (einschließlich der Catch-all-Regel `*`), der einzigen gemessenen _fail-closed_-Schicht — eine unbekannte Regel führt dazu, dass der Start verweigert wird, statt den Schutz unbemerkt zu entfernen.
- Um die OS-Sandbox **zu erzwingen**: `GROK_TRANSLATE_SANDBOX=read-only`. Der Start schlägt fehl, wenn das System sie nicht einhalten kann — genau das gewünschte Verhalten.

**Kontingent**: Der Grok-Pool ist **wöchentlich und gemeinsam** für Chat, Imagine und Voice, und kein Befehl ermöglicht es, ihn auszulesen. Eine Stapelverarbeitung kann daher deine Chat-Nutzung verringern, ohne dass dies angezeigt wird — daher die auf 2 begrenzte Parallelität und eine Warnung in `regen_translations.sh`.
...
> **Die sechs Provider-Flags schließen sich gegenseitig aus.** Früher war es stillschweigend möglich, zwei zu kombinieren; verwendet wurde dann der zuerst geprüfte Provider: Eine Übersetzung, die über das Abonnement-Kontingent angefordert wurde (`--use_codex`, `--use_grok_cli`), konnte dadurch ohne jede Warnung auf nutzungsbasierte Abrechnung umgestellt werden. `argparse` lehnt diese Kombination nun ab.
...
- `marker`: unsichtbare Markdown-Link-Referenzdefinition (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`), gefolgt von einem fetten Blockquote. Nativ auf GitHub/GitLab lesbar und beim Build von einem remark-Plugin auf Astro-Seite nutzbar, um ein stilisiertes Banner zu erzeugen (siehe Blog jls42.org).
