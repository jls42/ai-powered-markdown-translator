...
- **1.7** 새로운 기능 :
    - 번역 시 원본 파일명을 유지하는 옵션 `--keep_filename`
    ...
    - 시스템 프롬프트 개선 :
        ...
        - 템플릿 변수 `{variable}` 보호
        - 요청되지 않은 번역가 주석 금지
    - 364개 파일에서 성공적으로 테스트됨 (블로그 마이그레이션 jls42.org)
- **1.6** 새로운 기능 :
    ...
    - 더 빠르고 비용 절감형 모델을 사용하기 위한 경제 모드 (`--eco`)
    - 디렉터리를 탐색하지 않고 단일 파일 번역 (`--file`)
    - 새로운 간소화된 명명 패턴 : `{base}-{lang}.md`
    - 모델 이름이 포함된 이전 형식을 유지하는 옵션 `--include_model`
...
- **1.5** 개선 사항 :
    ...
    - **번역 프롬프트 최적화 :**
        - 직접 번역 및 번역 메모용 프롬프트가 명확성과 효율성을 위해 강화되었으며, 메타데이터와 특정 포맷 요소의 보존에 대한 상세 지침이 포함되었습니다.
    - **코드 리팩토링 :**
        - Mistral AI 클라이언트 초기화를 위해 `MistralClient`를 `Mistral` 클래스로 교체.
        - 가독성과 유지보수를 위해 import 재구성.
        - 번역 시 원본 형식을 보존하기 위한 텍스트 분할 및 코드 블록 처리 개선.
    - **출력 파일 관리 :**
        - 출력 파일명에서 모델과 언어를 반대로 배치 (예: `f"{base}-{args.target_lang}-{args.model}.md"`), 번역 정리 및 검색 용이성 향상.
    - **기타 개선 사항 :**
        - 불필요한 빈 줄 제거로 코드 정리.
        - 스크립트 구조와 가독성을 향상하기 위한 사소한 조정.
- **1.4** 새로운 기능 :
    - 번역을 위한 Anthropic의 Claude API 지원
    - 명확성과 효율성 향상을 위한 프롬프트 최적화
...
**이 문서는 gpt-5-mini 모델을 사용하여 fr 버전에서 ko 언어로 번역되었습니다. 번역 과정에 대한 자세한 내용은 https://gitlab.com/jls42/ai-powered-markdown-translator 를 참고하십시오.**
