...
- **Multi-Provider**: Suport pentru 4 API-uri (OpenAI, Mistral, Claude, Gemini) + CLI-ul Codex inclus în abonamentul ChatGPT
...
Pentru o instalare izolată de restul pachetelor dumneavoastră:
...
intenționat mai largi: nu impun nimic celorlalte pachete ale dumneavoastră.

### Instrumente de asigurare a calității (opționale, dar recomandate)

Proiectul utilizează [`pre-commit`](https://pre-commit.com) pentru a împiedica trimiterea de cod formatat necorespunzător, vulnerabil sau care conține un secret. Instalare:
...
Hook-uri active: ruff (lint+format), shellcheck (bash), prettier (markdown/yaml/json), Lizard (complexitate), detect-secrets (chei API), mypy (tipare progresivă), Opengrep (SAST), pip-audit (dependențe CVE), unittest. Consultați secțiunea _Quality / pre-commit_ din `CLAUDE.md` pentru detalii.

## Configurare

Creați un fișier `.env` **în directorul din care lansați
comanda** (este căutat acolo, apoi în directoarele părinte) sau
definiți variabilele de mediu:
...
### Traducerea prin abonamentul ChatGPT (`--use_codex`)
...
- **Refuzat în CI** (`CI` sau `GITHUB_ACTIONS` definit): autentificarea prin
  abonament nu este prevăzută pentru un runner partajat, iar OpenAI descurajează acest
  flux de lucru în depozitele publice. Utilizați o cheie API pe această cale.
- Variabile de mediu: `CODEX_BIN` (cale explicită către binar) și
  `CODEX_TIMEOUT` (secunde per segment, implicit `600`).

### Traducerea prin abonamentul Grok (`--use_grok_cli`)
...
**Izolare — citiți înainte de utilizare.** Acest provider este structural **mai
slab** decât `--use_codex`, iar acest lucru este asumat:

- Codex rulează în `--sandbox read-only`, o limită impusă de sistem.
- Sandbox-ul Grok **nu se poate aplica** pe multe sisteme Linux recente: AppArmor blochează spațiile de nume ale utilizatorilor fără privilegii începând cu Ubuntu 24.04, iar lista de blocare a socket-urilor runtime-ului containerului eșuează dacă
  `/run/podman` este în `0700`. Or, un profil **integrat** care nu se poate aplica pornește **neizolat, în tăcere**.
- Prin urmare, scriptul nu solicită niciun profil implicit și **nu revine niciodată în tăcere**: afișează un avertisment. Izolarea se bazează pe regulile `--deny` ale CLI-ului (inclusiv regula catch-all `*`), singurul strat măsurat
  _fail-closed_ — o regulă necunoscută determină refuzul pornirii, în locul eliminării protecției fără notificare.
...
**Cota**: grupul Grok este **săptămânal și partajat** cu Chat, Imagine și
Voice, iar nicio comandă nu permite citirea acestuia. Prin urmare, o procesare în lot poate
consuma utilizarea conversațională fără ca acest lucru să fie semnalat — de aici
...
| `--use_codex`            | Utilizează CLI-ul Codex pe baza cotei abonamentului ChatGPT               |
| `--use_grok`             | Utilizează API-ul xAI (Grok) — necesită `XAI_API_KEY`                      |
| `--use_grok_cli`         | Utilizează CLI-ul Grok pe baza cotei abonamentului Grok                   |
| `--force`                | Forțează retraducerea                                                  |
| `--keep_filename`        | Păstrează numele original al fișierului                                     |
| `--news`                 | Mod știri: protejează citatele EN, gestionează steagurile în funcție de limbă |
...
> **Cele șase flag-uri ale providerilor se exclud reciproc.** Combinarea a două dintre ele
> era anterior acceptată în tăcere și era rezolvată către primul testat: o
> traducere solicitată pe baza cotei abonamentului (`--use_codex`, `--use_grok_cli`)
> putea astfel ajunge la facturare la utilizare fără niciun avertisment.
...
- `marker` : definiție Markdown invizibilă a unei referințe de link (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`), urmată de un blockquote aldin. Lizibilă nativ în GitHub/GitLab și utilizabilă la build de un plugin remark în Astro pentru a produce o bară stilizată (cf. blogului jls42.org).
