...
Vertaler voor Markdown-bestanden die **OpenAI**, **Mistral AI**, **Claude (Anthropic)**, **Google Gemini** en **Grok (xAI)** gebruikt — via een API, via het quotum van een ChatGPT-abonnement (Codex) of Grok zonder facturering per gebruik, of via **OpenCode**, de open-source-agent, met de provider van jouw keuze: een lokaal model (Ollama), gratis, via een abonnement (GitHub Copilot…) of met een sleutel.
...
- **Multi-Provider**: 5 API's (OpenAI, Mistral, Claude, Gemini, Grok) + 2 CLI's via een abonnement, zonder facturering per gebruik — Codex (ChatGPT) en Grok — + OpenCode (open source, MIT) met elke provider die in OpenCode is geconfigureerd, inclusief een lokaal model
...
de 28 vertalingen en alle hulpmiddelen voor kwaliteitscontrole.
...
`requirements.txt` is een **volledig vastgezette lock**, die de geteste
omgeving exact weerspiegelt. De gepubliceerde grenzen in `pyproject.toml` zijn
bewust ruimer: ze leggen je andere pakketten geen beperkingen op.

### Hulpmiddelen voor kwaliteitscontrole (optioneel, maar aanbevolen)
...
Actieve hooks: ruff (lint+format), shellcheck (bash), prettier (markdown/yaml/json), Lizard (complexiteit), detect-secrets (API-sleutels), mypy (geleidelijke typering), Opengrep (SAST), pip-audit (CVE's in afhankelijkheden), unittest. Zie de sectie _Quality / pre-commit_ in `CLAUDE.md` voor details.

## Configuratie

De sleutels worden op **drie plaatsen** gezocht, van hoogste naar laagste prioriteit.
Elke plaats vult alleen aan wat bij de vorige ontbreekt.
...
zonder die te wijzigen. En een variabele die al in de omgeving is ingesteld, heeft
voorrang op beide:
...
Als er geen sleutel wordt gevonden, toont de opdracht geen call trace: ze vermeldt
...
`GROK_TRANSLATE_SANDBOX` (zie de sectie Grok CLI), `OPENCODE_BIN` /
`OPENCODE_TIMEOUT` (zie de sectie OpenCode) en `OPENROUTER_BASE_URL` /
`OPENROUTER_TIMEOUT` / `OPENROUTER_PREFLIGHT_TIMEOUT` (zie de sectie
OpenRouter). Voor
...
betaalde API) en `REGEN_JOB_TIMEOUT` (limiet per job, standaard 600 s, 1.800 s voor Codex).
...
Deze provider gebruikt geen enkele API-sleutel: hij bestuurt de officiële Codex CLI
in niet-interactieve modus, zodat de vertaling wordt afgetrokken van het quotum van
...
authenticeren aanroepen naar de API Platform niet en worden door dit script
overigens nooit gelezen.
...
de `PATH` en vervolgens het Python-pakket `openai-codex-cli-bin`. Dat laatste staat
bewust niet in `requirements.txt`: het is ~250 MB groot, wat anders aan alle gebruikers
zou worden opgelegd voor een optionele provider.

**Goed om te weten:**

- **Er wordt geen API-sleutel gebruikt.** `OPENAI_API_KEY` en `CODEX_API_KEY` worden
  uit de omgeving van het subproces verwijderd, zodat een sleutel in
  `.env` de vertaling nooit kan laten overschakelen op facturering per
  gebruik.
- **Eén segment = één „lokaal bericht”** binnen het venster van 5 uur van het abonnement.
  Gebruik `--eco` (model `gpt-5.6-luna`, 250-2.000 berichten/5 uur bij Plus)
  in plaats van het kwaliteitsmodel (`gpt-5.6-sol`, 10-100 berichten/5 uur).
- **Langzamer** dan een API-aanroep: reken op ~45 s voor een volledige README,
  tegenover enkele seconden bij een rechtstreekse aanroep.
- **Geweigerd in CI** (`CI` of `GITHUB_ACTIONS` ingesteld): het abonnement
  wordt geauthenticeerd via een persoonlijk sessiebestand, en dit naar een gedeelde
  runner overbrengen komt erop neer daar een identiteit te plaatsen die kan worden
  hergebruikt door alles wat erop wordt uitgevoerd. Gebruik hiervoor een API-sleutel.
...
- Het script vraagt daarom standaard geen profiel aan en **valt nooit stilzwijgend
  terug**: het toont een waarschuwing. De isolatie berust op de `--deny`-regels
  van de CLI (waaronder de catch-all `*`), de enige gemeten
  _fail-closed_-laag — een onbekende regel zorgt ervoor dat het opstarten wordt
...
- Om de OS-sandbox **verplicht te stellen**: `GROK_TRANSLATE_SANDBOX=read-only`. Het
  opstarten mislukt als de machine daaraan niet kan voldoen, wat het beoogde
  gedrag is.

**Quotum**: de Grok-pool is **wekelijks en gedeeld** met Chat, Imagine en
Voice, en geen enkele opdracht kan het uitlezen. Een batchverwerking kan dus
je gebruik voor gesprekken verminderen zonder dat dit ergens wordt gemeld —
vandaar een gelijktijdigheidslimiet van 2 en een waarschuwing in `regen_translations.sh`.
...
[OpenCode](https://opencode.ai) is een **open source (MIT)**-codeagent in de
...
een **lokaal** model. Deze provider stuurt `opencode run` aan in niet-interactieve modus en
beperkt de aanroep tot één enkele heen-en-teruginteractie, zonder enig hulpmiddel.
...
uitsluitend wat is geverifieerd.
...
provider en er wordt niet namens u een standaardwaarde gekozen: zijn eigen fallback
zou een gratis model zijn waarvan de interacties voor training kunnen worden gebruikt.
...
**Afscherming — wat het script bij elke aanroep doet:**

- Een inlineconfiguratie (`OPENCODE_CONFIG_CONTENT`), die voorrang heeft op die van
  u, definieert een agent `aipmt` waarvoor **alle hulpmiddelen worden geweigerd**
  (`permission: { "*": "deny" }`): het model kan niet lezen, schrijven of
  opdrachten uitvoeren — uit metingen blijkt dat het dit zelfs niet probeert. Het delen van sessies
...
- De aanroep draait in een **lege tijdelijke map**, met de schakelaars
  ...
  en uw `~/.claude/CLAUDE.md` — uit metingen bleek dat een instructie om “elk antwoord
  met BANANA te beëindigen” in een `AGENTS.md` op de vertaling werd toegepast. De
  globale regels van `~/.config/opencode/AGENTS.md` blijven daarentegen
  van toepassing: OpenCode staat niet toe ze uit te sluiten.
- Het uitvoercontract vereist alles tegelijk: exitcode 0, geen enkel
  `error`-event, geen aanroep van hulpmiddelen, een laatste stap die is voltooid met `stop`, niet-lege
  tekst en de agent die daadwerkelijk is geladen — een onbekende `--agent` laat
  OpenCode niet mislukken; het **valt stilzwijgend terug** op de codeeragent, met actieve
...
- **Geen enkele aipmt-sleutel wordt doorgegeven** aan het subproces (dezelfde filtering
  ...
  de sleutel van OpenCode zelf (Zen, Go). Providers worden geconfigureerd in
  OpenCode (`opencode auth login`, `opencode.json`), niet in het `.env` van aipmt.

**Goed om te weten:**

- **De gratis Zen-modellen zijn “stealth”- of bijdragersmodellen**,
  wisselend, met ongedocumenteerde limieten, en hun interacties kunnen voor
  ...
  doorgang; `opencode/big-pickle` is langzamer en twee gelijktijdige verzoeken bleven
  onbeantwoord.
- **Een lokaal model moet minstens 16 k context bieden** — segmenten bevatten
  maximaal 16.000 tekens — terwijl Ollama vaak standaard 4.096
  configureert. Met Ollama: een `Modelfile` met `PARAMETER num_ctx 32768`, gevolgd door
...
- `--eco` heeft geen effect (het model is dat van `--model`);
  ...
  worden opgegeven als het model dit kent.
...
- Omgevingsvariabelen: `OPENCODE_BIN` (expliciet pad naar het uitvoerbare bestand,
  ...
  (seconden per segment, standaard `600`). Als `OPENCODE_CONFIG` is
  geëxporteerd, wordt het niet door `aipmt` gelezen: het wordt ongewijzigd aan OpenCode doorgegeven, dat het respecteert.
...
`reasoningEffort: "none"` is geen detail: Ollama schakelt redeneren op deze
modellen standaard in en een Modelfile kan dit niet uitschakelen. Gemeten
via OpenCode: zonder de optie kost “De kat slaapt op het kleed” 919 tokens
aan redenering en 68 s; met de optie 9 tokens.
...
3 Engelse citaten beschermd door de modus `--news`), met dezelfde opdracht en drie
modellen:

| Model                                    | Duur         | Structuur                                                  | Afwijkingen                                                                                |
| ---------------------------------------- | ------------ | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `opencode/mimo-v2.5-free` (Zen, gratis) | 4 min 26 s   | identiek aan de bron                                       | geen                                                                                       |
| `ollama/gemma4-12b-32k` (lokaal)         | 10 min 10 s  | links, URL's, tabellen, tags, vet en inlinecode identiek   | één verzonnen citaatregel (🇺🇸 + parafrase), één dubbele bronvermelding                    |
| `ollama/qwen3.5-9b-32k` (lokaal)         | 8 min 18 s   | links, URL's, tabellen en tags identiek                    | één verzonnen citaatregel, enkele toegevoegde vetgedrukte teksten en inlinecodes, één segment opnieuw verwerkt |

Deze twee lokale modellen zijn sindsdien **afgekeurd**: één vrije interpretatie per artikel
volstaat om een model ongeschikt te maken voor gepubliceerde vertalingen. Vijf andere zijn
...
`gpt-oss:20b` is behouden — en zelfs dit model laat passages in het Frans staan in
een compact artikel; zie de tabel met aanbevolen modellen.
...
structuur, maar permitteert zich één vrije interpretatie per artikel, terwijl het gatewaymodel
dat geen enkele keer deed: nalezen vóór publicatie of alleen gebruiken voor concepten.

### Vertalen via OpenRouter (`--use_openrouter`)

OpenRouter is een **router** voor meer dan 400 modellen die door derden worden gehost,
waarbij gebruik uit één tegoed wordt afgerekend. Met één sleutel biedt het toegang tot modellen
die geen van de andere providers aanbiedt, met name open Chinese modellen.
...
Twee bijzonderheden van de routering waren bepalend voor de implementatie, en beide zijn
meetbaar:

- **Hetzelfde model wordt aangeboden door tientallen hostingproviders met
  verschillende limieten.** Voor `z-ai/glm-5.3-flash` zijn er 23 hostingproviders, waaronder één met een limiet van
  2.048 uitvoertokens: zonder voorzorgsmaatregel werd een lange vertaling in één van de 23 gevallen
  ...
  uitvoertokens of met een verslechterde status en zet vervolgens de overige vast met
  `allow_fallbacks: false` — zonder dit zou de router opnieuw een uitgesloten
  hostingprovider gebruiken.
- **Redeneren wordt tegen het uitvoertarief afgerekend.** Hetzelfde verzoek aan
  `z-ai/glm-5.2`, antwoord “OK”: 107 voltooiingstokens met de standaardinstelling van het model,
  2 met redeneren uitgeschakeld. Daarom wordt dit standaard uitgeschakeld bij modellen
  die dit toestaan. Modellen die redeneren verplicht stellen — `reasoning.mandatory`, 288 van de 431
  ...
  `max`, en daarmee werden de 32.768 uitvoertokens al vóór het einde van de
  vertaling opgebruikt. Het verhogen van de limiet zou niets hebben veranderd, omdat de inspanning er een
  percentage van toewijst. `--reasoning_effort` houdt voorrang en `none` bij een model
...
die aan de limiet voldoet, stopt de opdracht voordat er kosten worden gemaakt.

Andere punten:

- Het contextvenster komt uit de catalogus, niet uit een constante: de
  segmentatie past zich er daadwerkelijk aan aan, ook voor modellen met 4.095 tokens.
- `--eco` heeft geen effect (het model is dat van `--model`).
- `finish_reason=length` met lege uitvoer is geen afkapping, maar een
  budget dat door het redeneren is verbruikt; het bericht vermeldt dit, omdat beide
...
| `--use_opencode`         | OpenCode (open source) gebruiken via de in OpenCode geconfigureerde provider; vereist `--model provider/modèle` |
...
> **De acht providerflags sluiten elkaar wederzijds uit.** Het combineren van twee
> werd voorheen stilzwijgend geaccepteerd en koos de eerste die werd getest: een
> vertaling die via het abonnementsquotum was aangevraagd (`--use_codex`, `--use_grok_cli`)
> kon zo zonder enige waarschuwing op basis van gebruik worden afgerekend.
> `argparse` weigert de combinatie voortaan.

### Vertaalnotitie: posities en indelingen

Met `--add_translation_note` kan de translator de notitie bovenaan, onderaan of op beide plaatsen zetten en deze weergeven als eenvoudige tekstindeling (achterwaarts compatibel) of als `marker`-indeling die door een Markdown-plugin kan worden verwerkt.
...
- `legacy` (standaard): vetgedrukte alinea `**...**` — gedrag strikt identiek aan v1.8, byte-for-byte. Compatibel met Hugo, GitHub, GitLab en elke Markdown-renderer.
- `marker`: onzichtbare Markdown-linkreferentiedefinitie (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`), gevolgd door een vetgedrukt blockquote. Rechtstreeks leesbaar op GitHub/GitLab en tijdens het bouwen te verwerken door een remark-plugin aan de Astro-zijde om een gestileerde banner te produceren (zie blog jls42.org).
...
## Welke modellen houden stand

Een model dat één alinea goed vertaalt, behoudt niet noodzakelijk de structuur
van een volledig document. Deze metingen zijn afkomstig van **daadwerkelijk
uitgevoerde vertalingen**, met de opdracht die u hierboven zou lezen, op drie
documentensets en veertien doeltalen: en, es, de, it, pt, nl, pl, sv, ro, ja,
ko, zh, ar, hi.

Twee kolommen, die niet hetzelfde betekenen. **Geschreven** telt de
vertalingen die worden voltooid — de beveiligingen van het script tegen
stille fouten laten het bestand door. **Zonder afwijking** telt de vertalingen
waarvan de structuur identiek is aan de bron: dezelfde secties, dezelfde links,
dezelfde URL's, dezelfde blokken en inline codes, dezelfde tabellen, dezelfde
citaten, dezelfde vlaggen.

### Dicht blogartikel, modus `--news`

589 regels, 140 links, 21 secties, 3 beveiligde Engelse citaten. Dit is het
meest veeleisende van de drie documenten: de modus `--news` voegt boven op
de Markdown-structuur beperkingen voor vlaggen en citaten toe.
...
Twee batches werden **onderbroken wegens onvoldoende tegoed**, wat in hun
noemer zichtbaar is: `qwen3.5-27b` stopte bij negen talen, `kimi-k2.6` bij
vier — de laatste na een time-out van veertig minuten en twee weigeringen,
voor bijna $ 0,33 per taal.

Een methodologische kanttekening bij de OpenRouter-regels: ze zijn gemeten met
de **standaardinstellingen van de router**, voordat `--use_openrouter` bestond.
`z-ai/glm-5.2` is sindsdien opnieuw gemeten met de meegeleverde provider en
uitgeschakelde reasoning, en levert exact dezelfde 14/14 op. `z-ai/glm-5.3-flash`
faalde tweemaal doordat het uitvoerbudget bij de standaardinstelling van de
router was opgebruikt; de provider vraagt deze modellen voortaan om de laagste
reasoning effort die ze accepteren, en de controletest voor de problematische
talen slaagt.

### README van dit project, standaard-Markdown

508 regels, 219 inline codes, 40 afsluitingen van blokken, 45 tabelregels. Hier
is geen modus `--news`: de moeilijkheid komt door de hoge codedichtheid.
...
documenten zijn **gemakkelijker** dan de twee voorgaande, en dat blijkt uit de
tabel.
...
- **Drie modellen hebben op de twee dichte documenten nooit informatie
  verloren**: `gemini-3.7-flash`, `gpt-5.6-sol` via het ChatGPT-abonnement en
  `z-ai/glm-5.2` via OpenRouter. Hun enige afwijkingen in de standaardmodus
  zijn één of twee niet overgenomen `**`-paren in één of twee talen,
  nooit een URL, codeblok of citaat.
- **De onderscheidende factor is de dichtheid van het document, niet de modus
  `--news`.** Grok via een abonnement faalt 13 van de 14 keer bij het
  blogartikel en slaagt voor 14 van de 16 openbare README's: de oorzaak is dat
  het model bij een lang segment de draad kwijtraakt, zoals bevestigd door een
  controletest — de geïsoleerde passage wordt correct vertaald.
- **Niet-Latijnse schriften vormen niet de verwachte scheidslijn.**
  ...
  **en Roemeens**; Mistral en MiMo verliezen alleen inline codes bij
  niet-Latijnse schriften.
- **Reasoning uitschakelen gaat niet ten koste van de kwaliteit.**
  `z-ai/glm-5.2` verwerkt veertien talen zonder één afwijking onder beide
  omstandigheden — reasoning standaard actief via de router en vervolgens
  uitgeschakeld door `--use_openrouter` — met achttien keer minder gefactureerde
  uitvoertokens. Deze meting rechtvaardigt de standaardinstelling van de
  provider.
- **Een traag model is geen betrouwbaar model.** `deepseek-v4-flash-0731` doet 37
  minuten per taal over 4 van de 14 vertalingen, `qwen3.8-flash` 26 minuten
  over een bijna perfect resultaat en Gemini 1 minuut 18 over een foutloos
  resultaat.

### Wat deze tabel niet is

- **Dit is geen volledige ranglijst.** Alleen OpenRouter biedt al meer dan
...
- **Deze metingen hebben een datum**: 4 en 5 september 2026. Modellen veranderen
  ...
  er verschijnen elke week nieuwe modellen.
- **De doorlooptijden vormen geen ranglijst.** De parallelisatie varieerde
  afhankelijk van de testronde van 3 tot 6 gelijktijdige vertalingen en de
  doorvoer van een provider wisselt gedurende de dag. Ze geven een orde van
  grootte, geen vergelijking.
- **Een resultaat hangt evenzeer van het document als van het model af.**
