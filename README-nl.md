# AI-aangedreven Markdownvertaler
...
Vertaler van Markdown-bestanden met **OpenAI**, **Mistral AI**, **Claude (Anthropic)** en **Google Gemini**.
...
Het commando `aipmt` is dan overal beschikbaar. Als de map met
...
`requirements.txt` is een **volledig vastgepinde lock**, een exacte weerspiegeling van
de geteste omgeving. De gepubliceerde beperkingen in `pyproject.toml` zijn
...
Maak een bestand `.env` **in de map van waaruit je het
commando uitvoert** (daar wordt eerst gezocht en daarna in de bovenliggende mappen),
of definieer de omgevingsvariabelen:
...
en `GROK_TRANSLATE_SANDBOX` (zie de sectie Grok CLI). Aan de kant van
...
### Vertalen met een ChatGPT-abonnement (`--use_codex`)

Deze provider gebruikt geen API-sleutel: hij stuurt de officiële Codex CLI aan in
niet-interactieve modus. De vertaling wordt dus afgetrokken van het al betaalde
quotum van het ChatGPT-abonnement (Plus, Pro, Business…). Dit is de enige door
OpenAI gedocumenteerde manier voor dit gebruik — tokens van `~/.codex/auth.json`
...
- **Er wordt geen API-sleutel gebruikt.** `OPENAI_API_KEY` en `CODEX_API_KEY` worden
  uit de omgeving van het subprocess verwijderd. Zo wordt gegarandeerd dat een
  sleutel in `.env` de vertaling nooit laat overschakelen naar facturering
  op basis van gebruik.
- **Eén segment = één ‘lokaal bericht’** binnen het venster van 5 uur van het
  abonnement. Gebruik `--eco` (model `gpt-5.6-luna`, 250–2.000
  berichten/5 uur op Plus) in plaats van het kwaliteitsmodel (`gpt-5.6-sol`,
  10–100 berichten/5 uur).
- **Langzamer** dan een API-aanroep: reken op ongeveer 45 seconden voor een
  volledige README, tegenover enkele seconden rechtstreeks.
- **Geweigerd in CI** (`CI` of `GITHUB_ACTIONS` gedefinieerd): authenticatie
  ...
  workflow voor openbare repositories af. Gebruik op deze route een API-sleutel.
- Omgevingsvariabelen: `CODEX_BIN` (expliciet pad naar het binaire bestand) en
  `CODEX_TIMEOUT` (seconden per segment, standaard `600`).

### Vertalen met een Grok-abonnement (`--use_grok_cli`)
...
**Inperking — lezen vóór gebruik.** Deze provider is structureel **zwakker** dan
`--use_codex`, en dat is bewust:
...
- De sandbox van Grok kan op veel recente Linux-systemen niet worden toegepast:
  AppArmor blokkeert onbevoorrechte user namespaces sinds Ubuntu 24.04, en de
  denylist van containersockets faalt als `/run/podman` op `0700` staat.
  Een **ingebouwd** profiel dat niet kan worden toegepast, start daarentegen
  **stilzwijgend oningeperkt**.
- Het script vraagt daarom standaard geen profiel aan en valt **nooit stilzwijgend
  terug**: het toont een waarschuwing. De inperking berust op de regels
  `--deny` van de CLI (waaronder de catch-all `*`), de enige
  _fail-closed_ gemeten laag — een onbekende regel zorgt ervoor dat het starten
  wordt geweigerd in plaats van de bescherming stilzwijgend te verwijderen.
- Om de **OS-sandbox te vereisen**: `GROK_TRANSLATE_SANDBOX=read-only`. Het starten mislukt als de
  machine deze niet kan honoreren, wat het gewenste gedrag is.

**Quotum**: de Grok-pool is **wekelijks en gedeeld** met Chat, Imagine en
Voice, en er is geen commando om het uit te lezen. Een batchverwerking kan dus
je conversatiegebruik aanspreken zonder enige melding — vandaar een
concurrentie van maximaal 2 en een waarschuwing in `regen_translations.sh`.
...
| `--news` | Nieuwsmodus: EN-citaten beschermen, vlaggen per taal beheren |
...
> flags stilzwijgend geaccepteerd en werd de eerste geteste optie gebruikt: een
> vertaling die was aangevraagd met abonnementsquotum (`--use_codex`, `--use_grok_cli`)
> kon zo zonder enige waarschuwing worden gefactureerd op basis van gebruik.
...
plaatsen zetten, en haar weergeven als eenvoudige tekst (achterwaarts compatibel)
of als `marker`-opmaak die door een Markdown-plugin kan worden verwerkt.
...
- `both`: notitie bovenaan EN onderaan ingevoegd (één LLM-aanroep, inhoud hergebruikt voor beide plaatsingen).

**Opmaak** (`--note_format`):

- `legacy` (standaard): vetgedrukte alinea `**...**` — volledig identiek gedrag aan v1.8, byte voor byte. Compatibel met Hugo, GitHub, GitLab en elke Markdown-renderer.
- `marker`: onzichtbare Markdown-linkreferentiedefinitie (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`), gevolgd door een vetgedrukte blockquote. Native leesbaar op GitHub/GitLab en bij het bouwen bruikbaar door een remark-plugin aan de Astro-kant om een gestileerde banner te produceren (zie de blog jls42.org).
...
> `gemini-3.7-flash`) behoudt de Markdown-structuur getrouw bij scripts in niet-Latijnse
> talen (PL, JA, ZH, AR, HI), ook in `--news`-modus, waarin het behoud van
> placeholders belangrijk is. Gemeten op deze README, vertaald naar het Japans:
> identieke structuur aan `gemini-3.1-pro-preview` (21 lijsten, 18 codeblokken, 13 HTML-links,
> 13 afbeeldingen, alle URL's behouden) met ongeveer 6× minder latentie. OpenAI
