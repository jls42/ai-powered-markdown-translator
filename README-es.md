...
Traductor de archivos Markdown que utiliza **OpenAI**, **Mistral AI**, **Claude (Anthropic)**, **Google Gemini** y **Grok (xAI)** — mediante API o con la cuota de una suscripción a ChatGPT (Codex) o Grok, sin facturación por uso.
...
- **Multi-Provider**: 5 API (OpenAI, Mistral, Claude, Gemini, Grok) + 2 CLI con suscripción, sin facturación por uso — Codex (ChatGPT) y Grok
...
| 2   | `.env` del directorio actual (o de un directorio padre) | una clave propia del proyecto            |
| 3   | `~/.config/aipmt/.env`                        | **instalado una vez, sirve en todas partes**   |

Lo más sencillo después de un `pip install` es utilizar la tercera:
...
(si no, se ignora, como prescribe la especificación), y `%APPDATA%`
en Windows.
...
y `GROK_TRANSLATE_SANDBOX` (consulta la sección de Grok CLI). Para
...
### Traducir con la suscripción a ChatGPT (`--use_codex`)
...
y después el paquete Python `openai-codex-cli-bin`. Este último no se incluye deliberadamente
...
- **Más lento** que una llamada a la API: calcula unos 45 s para un README completo, frente
  a unos pocos segundos directamente.
...
### Traducir con la suscripción a Grok (`--use_grok_cli`)

Mismo principio que `--use_codex`, con el CLI oficial **Grok Build**:
la traducción se descuenta de la suscripción a Grok (SuperGrok / X Premium+) en lugar
...
**Aislamiento — leer antes de utilizar.** Este proveedor es estructuralmente **más
...
- El sandbox de Grok **no puede aplicarse** en muchos equipos Linux
  ...
  aplicarse se inicia **sin aislamiento, en silencio**.
- Por tanto, el script no solicita ningún perfil de forma predeterminada y **nunca
  recurre silenciosamente**: muestra una advertencia. El aislamiento se basa en las
  reglas `--deny` del CLI (incluido el catch-all `*`), la única capa
  comprobada _fail-closed_ — una regla desconocida hace que se rechace el inicio en
  lugar de retirar la protección sin avisar.
...
Voice, y no existe ningún comando que permita consultarlo. Por tanto, un procesamiento
por lotes puede consumir tu uso conversacional sin que nada lo indique — de ahí una
...
> **Las seis flags de proveedor son mutuamente excluyentes.** Antes se aceptaba
> combinar dos en silencio y se resolvía según el primero probado: una traducción
> solicitada con la cuota de una suscripción (`--use_codex`, `--use_grok_cli`)
...
Con `--add_translation_note`, el traductor puede colocar la nota arriba, abajo o en ambas posiciones,
y generarla en formato de texto simple (compatible con versiones anteriores) o en formato
`marker` consumible por un plugin de Markdown.
...
- `legacy` (valor predeterminado): párrafo en negrita `**...**` — comportamiento estrictamente idéntico al de v1.8, byte por byte. Compatible con Hugo, GitHub, GitLab y cualquier renderer de Markdown.
- `marker`: definición invisible de referencia de enlace Markdown (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`) seguida de un blockquote en negrita. Legible de forma nativa en GitHub/GitLab y utilizable durante la compilación mediante un plugin remark de Astro para producir un banner estilizado (consulta el blog jls42.org).
