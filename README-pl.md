...
bloki kodu, kod w tekście, adresy URL, kotwice, tabele oraz front matter.
Dziesięć sposobów wywołania modelu — pięć API, trzy subskrypcje bez rozliczania
za użycie, dwa routery — oraz opublikowany wynik pomiaru tego, co każdy model
naprawdę zachowuje.

## W skrócie

- **Dziesięć ścieżek dostawców**: API OpenAI, Mistral, Claude, Gemini i Grok;
  subskrypcje ChatGPT (Codex), Grok i Google (Antigravity) bez opłat za
  użycie; routery OpenCode (open source, darmowe lub lokalne) oraz OpenRouter
  (ponad 400 modeli).
- **Zero błędów z powodu utraconego tokenu**: bloki kodu, kod w tekście,
...
- **Długie dokumenty**: segmentacja dostosowana do okna kontekstowego modelu.
- **Tryb `--news`**: chronione angielskie cytaty i flagi zarządzane według
  języka, dla artykułów z przeglądem informacji.
...
| 2   | `.env` w bieżącym katalogu (lub nadrzędnym) | klucz specyficzny dla danego projektu |
| 3   | `~/.config/aipmt/.env`                                 | instalowane raz, działa wszędzie      |
...
`GEMINI_API_KEY` jest akceptowany zamiast `GOOGLE_API_KEY`. Plik użytkownika
podlega `XDG_CONFIG_HOME` (tylko ścieżka bezwzględna) oraz `%APPDATA%`
w systemie Windows. Bez klucza polecenie wylistuje wszystkie trzy lokalizacje.

**Plik `.env` projektu nie może ani przekierowywać wywołań, ani wybierać
...
`APPDATA` są tam ignorowane z ostrzeżeniem. Sklonowane repozytorium nie może
mieć możliwości przechwycenia Twojego klucza ani zmuszenia Cię do uruchomienia
własnego programu przy pierwszym tłumaczeniu. Plik ten jest również odczytywany bez
interpolacji: `NOM=${OPENAI_API_KEY}` nie kopiuje w nim klucza. Umieść te zmienne
w środowisku lub w `~/.config/aipmt/.env`.
...
`OPENROUTER_PREFLIGHT_TIMEOUT` (domyślnie 30). Każda z nich została szczegółowo opisana
w sekcji odpowiedniego dostawcy.
...
`document.md` przetłumaczone na język hiszpański daje `document-es.md` w `--target_dir`;
z `--include_model`, `document-es-gpt-5.6-terra.md`. Rozszerzenie zawsze
zmienia się na `.md` — `article.mdx` daje `article-en.md` — z wyjątkiem
`--keep_filename`, które zachowuje oryginalną nazwę. Istniejące tłumaczenie
jest pomijane bez podania `--force`.

Kody wyjścia: `0`, jeśli wszystko się powiodło lub zostało pominięte, `1`, jeśli co najmniej jeden plik
zakończył się niepowodzeniem (lista na wyjściu błędów), `2`, jeśli przyczyną jest konfiguracja.
Plik zakończony niepowodzeniem nigdy nie jest zapisywany, nawet jeśli sam zapis się nie powiedzie:
zawartość jest zapisywana obok, a następnie zmieniana jest jej nazwa. Wystarczy ponowić próbę.
...
| Claude Sonnet 5      | klucz API Anthropic               | ⚠️ 11/14                  | ⚠️ 12/14     | 3 języki w artykule: pojawił się blok kodu (es, de, hi); 2 w tym README: link bez formatowania (sv), pogrubione słowo (zh)          |
| Qwen 3.7 Flash       | klucz OpenRouter                  | ❌ 8/14                   | ⚠️ 10/14     | 1 język odrzucony w artykule, 5 innych ma odchylenia; w tym README około 40 słów umieszczonych w `code` (ar)                 |
| Grok 4.6             | subskrypcja Grok                  | ❌ 8/14                   | nieoceniane  | 5 języków odrzuconych na 14 z powodu braku zwróconego kodu w tekście i adresów URL; niderlandzki różni się we wszystkim            |
| GPT-OSS 20B          | model lokalny (Ollama)            | ❌ 7/14                   | nieponowiono | 4 języki odrzucone na 14: model pozostawił fragmenty po francusku, strażnik je zatrzymał                                           |
| MiMo v2.5 (darmowy)  | OpenCode Zen, bez konta           | ❌ 11/14                  | nieponowiono | 1 język odrzucony; utracona sekcja w języku polskim                                                                                  |
...
| DeepSeek V4 Flash    | klucz OpenRouter                  | ❌ 3/14                   | nieponowiono | 10 języków odrzuconych na 14; 37 minut na język                                                                                     |

|     | Co oznacza symbol                                                                                                                                                                                     |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅  | wszystkie czternaście języków przetłumaczonych i nic nie różni się od źródła                                                                                                                          |
| ⚠️  | wszystkie czternaście języków przetłumaczonych; to, co się różni, to **formatowanie** — pogrubione słowo, `code`, link tracący nawiasy. Nie brakuje tekstu, URL, bloku kodu ani sekcji      |
...
- **Odrzucone tłumaczenie to nie uszkodzone tłumaczenie.** Kiedy po powrocie
  brakuje tokenu, plik nie zostaje zapisany, a język jest liczony jako
  odrzucony. To właśnie przydarzyło się modelowi Grok w artykule: cztery kody
  w tekście i trzy adresy URL utracone już w pierwszym segmencie, we wszystkich
  pięciu systemach pisma nielacińskiego.
- **Ta siatka bezpieczeństwa nie obejmuje nagłówków, tabel, front matter ani
  ...
  bez wahania — tak jest w przypadku modelu Mistral. Elementów tych nie można
  zastąpić tokenem, a obecne strażniki ich nie weryfikują;
...
- **Grok nie ma oceny dla tego README**: jego sesja CLI wygasła po dwunastu
  językach, z czego jedenaście bez żadnych odchyleń. Przerwane testy nie są
  oceniane.
- **Gęstość dokumentu ma większe znaczenie niż język.** Grok radzi sobie ze
  zwykłymi plikami README i gubi się przy artykule gęstym od linków, w tym
  w języku niderlandzkim.

Daty i dokumenty: kolumna „Ten README” została zmierzona 9 września 2026 r.
na zamrożonej rewizji tego pliku (785 linii, 285 fragmentów kodu w tekście, 89 linii
tabeli), od tego czasu modyfikowanej — z wyjątkiem wiersza Antigravity, zmierzonego
26 września na rewizji opublikowanej z wersją 1.14.0, krótszej (600 linii,
257 fragmentów kodu w tekście, 85 linii tabeli). Kolumna „Gęsty artykuł
przeglądowy” pochodzi z testów przeprowadzonych 4 i 5 września na artykule o 589 liniach,
z wyjątkiem wiersza Grok, zmierzonego ponownie 9 września na innym wydaniu tego samego
przeglądu, oraz wiersza Antigravity, zmierzonego 26 września na tym samym artykule.
Kompletne tabele, czasy trwania oraz protokół znajdują się w
...
| `--news`                 | Tryb wiadomości: chroni cytaty EN, zarządza flagami według języka                                             |
...
| OpenCode    | `--model provider/modèle` wymagane                              | jw. — `--eco` bez wpływu |
| OpenRouter  | `--model fournisseur/modèle` wymagane                              | jw. — `--eco` bez wpływu |

### W ramach subskrypcji ChatGPT: `--use_codex`

Steruje oficjalnym CLI Codex: tłumaczenie jest odliczane z limitu
subskrypcji ChatGPT, bez klucza API i bez naliczania opłat za użycie.
...
- Każdy segment kosztuje co najmniej jedną „wiadomość” z 5-godzinnego okna — dwie,
  jeśli jego walidacja się nie powiedzie i zostanie ponowiony. OpenAI podaje
  szacunkowo 250–2000 wiadomości/5 godz. dla `gpt-5.6-luna` (`--eco`) oraz
...
- Wolniejsze niż API, a różnica rośnie wraz z wielkością dokumentu: w przypadku tego pliku README
  mediana wynosi 6 min 46 s na język przy użyciu `gpt-5.6-sol`, w porównaniu do 36 s dla
  `gemini-3.7-flash`.
- Odrzucane w środowisku CI (gdy zdefiniowano `CI` lub `GITHUB_ACTIONS`): subskrypcja
...
- **Słabsza izolacja niż w Codex.** Piaskownica systemu operacyjnego (sandbox OS) Groka nie ma zastosowania
  na wielu nowoczesnych stacjach roboczych Linux (AppArmor, gniazda środowiska uruchomieniowego
  kontenerów), a profil, którego nie można zastosować, uruchamia się po cichu bez izolacji.
  Skrypt domyślnie nie żąda więc żadnego profilu, informuje o tym i
  polega na regułach `--deny` CLI, w tym na ogólnej regule przechwytującej (catch-all) `*` — jedynej
  warstwie, która odmawia uruchomienia zamiast po cichu rezygnować z ochrony.
  `GROK_TRANSLATE_SANDBOX=read-only` wymusza piaskownicę systemu operacyjnego, a uruchomienie
  kończy się niepowodzeniem, jeśli maszyna nie może jej zapewnić.
- Limit jest tygodniowy, współdzielony z Chat, Imagine oraz Voice, i żadne
  polecenie nie pozwala na jego odczyt: partia zadań może uszczuplić limit konwersacyjny
...
Ta sama zasada w przypadku `agy`, oficjalnego CLI Antigravity: dla użytkowników opłacających Google
AI Pro lub Ultra tłumaczenie jest odliczane z limitu subskrypcji zamiast
rozliczania za tokeny. To jedyna ścieżka do tego limitu: Gemini CLI nie
...
- **Żadna płatna ścieżka nie pozostaje otwarta.** agy otrzymuje z Twojego
  ...
  kilka z jego zmiennych przełącza wywołanie bez wyświetlania czegokolwiek (zmierzone:
  jedna wysyła dokument do bramki innej firmy, inna do płatnego projektu
  Google Cloud), a czarna lista pomijała kolejne przy każdej rewizji.
  Przed jakimkolwiek segmentem polecenie `agy -p /config`, które nie zużywa limitu, musi wykazać,
  że płatne kredyty AI są wyłączone, bez klucza API i projektu Google Cloud — brak
...
- **Izolacja.** Każde wywołanie działa w prywatnym, tymczasowym katalogu
  domowym, z agentem tłumaczącym bez narzędzi: Twoje ustawienia, reguły,
  wtyczki, serwery MCP i hooki agy nie mają do niego dostępu, nic nie trafia do
  historii, a dane logowania pozostają w pęku kluczy, którego aipmt nigdy nie czyta.
  W przypadku nieodnalezienia agenta agy po cichu powraca do swojego agenta kodującego
  i jego narzędzi: cały wiersz dziennika musi potwierdzać właściwego agenta — dokument
  cytujący ten komunikat go nie zastępuje — w przeciwnym razie następuje odmowa.
- **Platformy**: Linux, w sesji posiadającej pęk kluczy (magistrala sesyjna
  D-Bus, Secret Service); macOS jest akceptowany, choć nie był tam testowany. Odrzucane
  w systemie Windows, gdzie agy nie odczytuje zmiennych izolujących poszczególne wywołania, oraz
  w systemie Linux bez magistrali sesyjnej — sesja SSH, kontener, serwer: agy
  ...
  minuty oczekiwania na kod logowania.
- **Modele**: te z `agy models`. Modele Gemini zawierają poziom wysiłku w swojej nazwie
  (`gemini-3.7-flash-low`…): nazwa bez sufiksu jest odrzucana przed wywołaniem, a
  `--reasoning_effort` nie przynosi efektu. Obie wartości domyślne zostały ustalone na podstawie
  testów w czternastu językach (por. [Szczegółowe pomiary](#szczegółowe-pomiary)).
  Claude i GPT-OSS mają własny limit, znacznie mniejszy: około 1% 5-godzinnego
  okna na zmierzone wywołanie, w porównaniu do 0,05% w przypadku Flash.
- **Limit**: według grup, okno 5-godzinne oraz tygodniowe,
  proporcjonalnie do kosztu w tokenach. Zmierzone na koncie autora na podstawie
  serii 32 tłumaczeń: około pół punktu procentowego 5-godzinnego okna
  dla pliku README o wielkości 40 000 znaków w `gemini-3.7-flash-medium`; limit
  tygodniowy zależy od poziomu subskrypcji. Ponawianie prób opiera się na tym, co agy oznacza
  jako możliwe do ponowienia; w przeciwnym razie wyczerpane okno nigdy nie jest ponawiane: powoduje
  błąd każdego pliku aż do zresetowania limitu wskazanego przez `/usage`.
- **Wolniejsze niż API**: w przypadku gęstego artykułu z pomiarami Gemini 3.7 Flash
  osiąga medianę 3 min 14 s na język w ramach subskrypcji, w porównaniu do 1 min 18 s przez
  API.
- **Przerwanie**: Ctrl-C lub zamknięcie terminala zatrzymuje agy wraz z
  ...
  dotyczy Codex, Grok CLI i OpenCode. W `nohup` tłumaczenie jest kontynuowane.
- Odrzucane w CI (gdy zdefiniowano `CI` lub `GITHUB_ACTIONS`): sesja logowania znajduje się
...
**Warunki korzystania z usługi: korzystasz na odpowiedzialność własnego konta.**
...
za pośrednictwem oprogramowania innych firm przy użyciu sesji Antigravity — wymieniono tam Claude Code,
OpenClaw i OpenCode — pod rygorem zawieszenia konta. aipmt
...
uruchamianie `agy -p` z lokalnego skryptu do własnej pracy
...
żaden zapis nie rozstrzyga jednoznacznie przypadku narzędzia dystrybuowanego takiego jak to.
...
produktów i uczenia maszynowego Google oraz podlegać weryfikacji przez
ludzi, również w przypadku płatnej subskrypcji. Rezygnacja (opt-out) wymaga ustawienia
`enableTelemetry`, o nieudokumentowanym działaniu, którego aipmt nie ustawia; Twoje ustawienia
agy nie są przenoszone do jego izolowanego środowiska. Nie przetwarzaj za jego pomocą żadnych poufnych danych.
...
bramka OpenCode Zen (darmowe modele, bez konta) lub model lokalny. W tym dokumencie
zmierzono kompleksowo dwie ścieżki: Zen oraz Ollama.
...
`--model` jest wymagane: bez tego OpenCode powróciłby do darmowego modelu,
w którym wymiana danych może służyć do trenowania, a wybór ten nie jest dokonywany
za Ciebie.

Izolacja przy każdym wywołaniu:

- wbudowana konfiguracja (inline), mająca pierwszeństwo przed Twoją, definiuje agenta `aipmt`,
  któremu odmawia się dostępu do wszystkich narzędzi (`permission: { "*": "deny" }`), współdzielenie
  sesji jest wyłączone, `--pure`, nigdy `--auto`;
- tymczasowy i pusty katalog roboczy, ustawione `OPENCODE_DISABLE_PROJECT_CONFIG` oraz
  `OPENCODE_DISABLE_CLAUDE_CODE` — bez nich OpenCode wstrzykuje do
  promptu `AGENTS.md` z bieżącego katalogu oraz `~/.claude/CLAUDE.md`.
  Globalny `~/.config/opencode/AGENTS.md` nadal jest wstrzykiwany, OpenCode nie pozwala
  na jego wykluczenie;
- kontrakt wyjściowy: kod wyjścia 0, brak zdarzeń `error`, brak wywołań
  narzędzi, ostatni krok jako `stop`, niepusty tekst i rzeczywiście
  załadowany agent `aipmt` — nieznany `--agent` nie powoduje błędu OpenCode, lecz
  po cichu powraca do agenta kodującego;
...
- Darmowe modele Zen ulegają zmianom, mają nieudokumentowane limity, a
  przesyłane do nich dane mogą być wykorzystywane do trenowania: nadają się do publicznej
  dokumentacji, ale nie do treści prywatnych.
- Model lokalny musi oferować co najmniej 16 tys. tokenów kontekstu, ponieważ segmenty
  mają do 16 000 znaków. Ollama często domyślnie konfiguruje 4096: należy użyć
  `Modelfile` z `PARAMETER num_ctx 32768`.
- `--eco` nie ma wpływu; `--reasoning_effort` jest przekazywany bez zmian jako
  `--variant` programu OpenCode.
...
modelach i którego Modelfile nie może wyłączyć. Zmierzone na zdaniu złożonym z
sześciu słów: 919 tokenów rozumowania i 68 sekund bez tej opcji, 9 tokenów z nią.

### Do ponad 400 modeli: `--use_openrouter`

OpenRouter to router rozliczany według zużycia z pojedynczego salda punktów (kredytów), działający przed
...
`--model` jest wymagane. Wstępne sprawdzenie (preflight), wykonywane przed jakimkolwiek naliczeniem opłat, obsługuje
dwie specyfiki routingu:

- **Ten sam model jest obsługiwany przez dziesiątki dostawców hostingu o różnych limitach** —
  w przypadku `z-ai/glm-5.3-flash` to 23 dostawców, w tym jeden z limitem
  2048 tokenów wyjściowych. Preflight odczytuje `/api/v1/models/{modèle}/endpoints`,
  odrzuca dostawców z limitem poniżej 8000 tokenów wyjściowych lub ze statusem obniżonej jakości (degraded), a
  pozostałych przypina za pomocą `allow_fallbacks: false`.
- **Rozumowanie jest rozliczane według stawki tokenów wyjściowych** — 107 tokenów w porównaniu do 2 przy
  odpowiedzi „OK” z `z-ai/glm-5.2`. Domyślnie jest ono wyłączone; modele,
  które go wymagają, otrzymują najniższy akceptowany przez nie poziom wysiłku, ponieważ domyślna wartość
...
- Okno kontekstu pochodzi z katalogu. Model mający poniżej 16 400 tokenów jest
  odrzucany przed jakimkolwiek wywołaniem: 8 400 na prompt i segment oraz co najmniej
  8 000 na wyjście.
- Brakujący identyfikator (slug) w katalogu, niedostępny katalog lub brak
  dostawcy hostingu spełniającego limit zatrzymują polecenie.
- `finish_reason=length` z pustym wyjściem oznacza budżet wyczerpany przez
  rozumowanie, a nie obcięcie: komunikat wyraźnie to rozróżnia.
- `--eco` nie ma wpływu.
- Zmienne: `OPENROUTER_API_KEY` (<https://openrouter.ai/keys>),
  `OPENROUTER_BASE_URL` (domyślnie `https://openrouter.ai/api/v1`, wymagane
  `https://`), `OPENROUTER_TIMEOUT` (domyślnie 900), `OPENROUTER_PREFLIGHT_TIMEOUT`
  (domyślnie 30).
...
po której następuje pogrubiony cytat blokowy: czytelna na GitHubie, możliwa do przetworzenia podczas budowania przez
wtyczkę remark.
...
**Zapisane** zlicza pliki przepuszczone przez mechanizmy ochronne (guards); **Bez
różnic** to te, w których `scripts/compare_structure.py` nie wykazuje żadnych różnic — taka sama liczba
sekcji, podtytułów, linków, unikalnych adresów URL, bloków kodu,
kodu w tekście (inline), wierszy tabeli, bloków cytatów i pogrubionych słów.

„Bez różnic” oznacza „nic nie wykryto”, a nie „identyczne”: narzędzie porównujące
zlicza elementy bez czytania ich zawartości. Nie zgłasza ono ani usunięcia nagłówka
poziomu 4, ani zastąpienia tekstu kodu w tekście, ani zamiany flagi, jak również nie ocenia poprawności językowej.

### Gęsty artykuł przeglądowy, tryb `--news`

Wydanie [przeglądu AI z jls42.org](https://jls42.org/fr/news):
589 wierszy, 140 linków, 21 sekcji, 3 chronione cytaty w języku angielskim. Kampania
...
Model Grok został zmierzony ponownie 9 września na innym wydaniu tego samego przeglądu
(356 wierszy): 9 zapisanych języków na 14, 8 bez rozbieżności. To właśnie ta wartość
widnieje w głównej tabeli. Trzy przerwane kampanie nie zostały
uwzględnione: `qwen3.5-27b` (9 języków) i `kimi-k2.6` (4) z powodu braku środków,
`z-ai/glm-5.3-flash`, w przypadku którego dwa niepowodzenia wynikały z ustawienia wnioskowania,
poprawianego od tego czasu przez dostawcę. Wiersze OpenRouter mierzono przy
domyślnych ustawieniach routera, przed `--use_openrouter`; `z-ai/glm-5.2`,
zmierzony ponownie z dostarczonym providerem, daje taki sam wynik 14/14. Liczby zostały
przeliczone 10 września przy użyciu obecnego narzędzia porównującego: `qwen3.8-flash` i
`qwen3.7-flash` zyskują po jednym języku w stosunku do pierwszej
publikacji, pozostałe pozostają bez zmian.

Wiersz `--use_antigravity` zmierzono 26 września na tym samym artykule,
przy czterech równoległych tłumaczeniach. W języku angielskim model sam usunął
trzy wiersze francuskiego tłumaczenia pod cytatami, nie wymyślając
flagi, a angielskie cytaty pozostały nienaruszone: zapasowe czyszczenie nie
miało nic do zrobienia. W `--eco` (`gemini-3.7-flash-low`), dla zaledwie czterech
języków (en, ja, ar, hi): 4 zapisane z 4, wszystkie bez rozbieżności, mediana
1 min 52 s.

### README tego projektu, standardowy Markdown

Wersja zamrożona 9 września 2026 r.: 785 wierszy, 285 fragmentów kodu inline, 40
zamknięć bloków, 89 wierszy tabeli. Cztery tłumaczenia równoległe.

| Model                                           | Zapisane| Bez rozbieżności | Mediana/język | Czym się różni                                                           |
| ----------------------------------------------- | ------- | ---------------- | ------------- | ------------------------------------------------------------------------ |
...
| `qwen/qwen3.7-flash`                            | 14/14   | ⚠️ 10/14         | 2 min 17 s    | 40 dodanych kodów inline w języku arabskim; pogrubienie (hi, ja, ko)     |
| `mistral-large-latest`                          | 14/14   | ❌ 1/14          | 2 min 44 s    | jedna utracona sekcja (ar, hi, ko); dodane bloki kodu (ja, ko, ro, zh)   |

Dwie przerwane kampanie nie zostały uwzględnione: Grok – sesja CLI wygasła
po dwunastu językach (jedenaście bez rozbieżności), oraz `qwen3.8-flash` – błąd HTTP 429 od
hosta po dwóch. `opencode/mimo-v2.5-free` i `ollama/gpt-oss-20b-32k`
nie były ponownie mierzone na tej wersji; w wersji z 4 i 5 września,
krótszej o 277 wierszy, każdy z nich zapisał po 9 tłumaczeń na 14, z czego
odpowiednio 7 i 1 bez rozbieżności.

Wiersz `--use_antigravity` nie był mierzony na zamrożonej wersji, lecz
26 września na wersji opublikowanej wraz z 1.14.0: 600 wierszy, 257 fragmentów kodu inline,
30 zamknięć bloków, 85 wierszy tabeli. Ponieważ jest krótsza o 185 wierszy,
nie można jej bezpośrednio porównywać z pozostałymi wierszami.

### Cztery pliki README znanych projektów

FastAPI, Ollama, tldr-pages i Vue.js, pobrane wprost z GitHuba —
dokumenty prostsze niż dwa poprzednie. Kampania miała na celu przetestowanie modeli
mających trudności; Gemini służy tu za punkt odniesienia.
...
- **Czas trwania ma charakter orientacyjny**: od trzech do sześciu równoległych
  tłumaczeń w zależności od kampanii, a przepustowość dostawcy zmienia się w ciągu dnia.
- **To obserwacje powiązane z konkretnym momentem**: modele zmieniają się pod tą samą nazwą, a Państwa
...
Oba wiersze są niezbędne: bez `pip install -e .`, `python -m aipmt`
odpowiada `No module named aipmt`.

Narzędzia kontroli jakości, opcjonalne, lecz zalecane:
...
Wszystkie 28 tłumaczeń w repozytorium (README i CHANGELOG, czternaście języków) można
wygenerować ponownie za pomocą `./regen_translations.sh --force` — Codex i `gpt-5.6-sol` domyślnie
na subskrypcji ChatGPT, cztery równolegle. `REGEN_PROVIDER` i
`REGEN_MODEL` zmieniają ścieżkę: `antigravity` pozostaje na subskrypcji,
...
- **[jls42.org](https://jls42.org)** — blog osobisty publikowany w 15 językach. Jego
  [codzienny przegląd AI](https://jls42.org/fr/news) jest tłumaczony każdego dnia
  przez to narzędzie i służy jako dokument referencyjny do powyższych pomiarów.
...
określonych w punktach 15 i 16 licencji GPL v3: dostarczany w stanie „takim, w jakim jest”, bez gwarancji przydatności
...
- **Przeczytaj ponownie przed publikacją.** Zabezpieczenia obejmują bloki kodu,
  kod inline, adresy URL, kotwice oraz cytaty w trybie `--news` — nie obejmują
  nagłówków, tabel, front matter ani sensu Twoich zdań.
- **Twoje dokumenty trafiają do wybranego dostawcy**, zgodnie z jego regulaminem
  i polityką przetwarzania danych. Niektóre darmowe modele mogą
  ponownie wykorzystywać Twoje zapytania do trenowania, a warunki usługi Antigravity
  pozwalają firmie Google na ich ponowne wykorzystanie i weryfikację przez ludzi,
  w tym w ramach płatnej subskrypcji; model lokalny to jedyny sposób,
  aby żadne dane nie opuściły Twojego komputera.
- **Wywołania API są płatne.** Ten program nie nakłada limitu
  wydatków: długi dokument, ponowienie próby po błędzie lub model, który intensywnie
  wnioskuje, generują wyższe koszty.
- **Opublikowane pomiary to obserwacje z określonego momentu**, a nie gwarancje.

Wymienione nazwy produktów i firm należą do ich odpowiednich
właścicieli. Ten projekt nie jest powiązany z żadną z nich.
