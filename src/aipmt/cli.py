"""Ligne de commande `aipmt` : arguments, validation des chemins, exécution.

`main` est le point d'entrée déclaré dans `pyproject.toml` et celui de
`python -m aipmt`. Les flags des providers viennent du registre ; le reste
(entrées/sorties, langues, nommage, note, mode news) est déclaré ici. Une
erreur de configuration (clé absente) s'affiche sans trace d'appel — le filet
est étroit à dessein : envelopper toute l'exécution masquerait un vrai bug.
"""
...
from .providers.registry import _add_provider_args, _select_provider_client
