...
- **스마트 세분화**: 모델별 토큰 제한에 따른 긴 텍스트 처리
- **코드 보존**: 코드 블록과 인라인 코드(`` `...` ``) 보존
- **파일 이름**: 원래 이름을 유지하기 위한 `--keep_filename` 옵션
- **뉴스 모드**: 뉴스 기사에서 영어 인용문을 보호하고 국기 처리를 관리하는 `--news` 옵션
...
이제 어디서나 `aipmt` 명령을 사용할 수 있습니다. Python 스크립트 디렉터리가
`PATH`에 없다면 `python -m aipmt`이 정확히 같은 작업을 수행합니다.
...
개발하려면 복제한 저장소가 여전히 필요합니다. 테스트,
28개의 번역 및 모든 품질 도구가 이곳에 있습니다.
...
`pyproject.toml`에 게시된 범위는 의도적으로 더 넓으며, 다른 패키지에는 아무런 제약을 가하지 않습니다.

### 품질 도구(선택 사항이지만 권장)

이 프로젝트는 서식이 잘못되었거나 취약하거나 비밀 정보가 포함된 코드의 커밋을 방지하기 위해 [`pre-commit`](https://pre-commit.com)을 사용합니다. 설치:
...
활성화된 Hook: ruff (lint+format), shellcheck (bash), prettier (markdown/yaml/json), Lizard (복잡도), detect-secrets (API 키), mypy (점진적 타이핑), Opengrep (SAST), pip-audit (CVE deps), unittest. 자세한 내용은 `CLAUDE.md`의 _Quality / pre-commit_ 섹션을 참조하세요.

## 구성

키는 **우선순위가 높은 곳부터 낮은 곳까지 세 곳**에서 검색됩니다.
각 위치는 이전 위치에서 비어 있는 부분만 채웁니다.

|     | 위치                                            | 용도                             |
| --- | --------------------------------------------- | ------------------------------------- |
| 1   | 환경 변수                     | CI, 컨테이너, 일시적인 재정의 |
| 2   | 현재 디렉터리(또는 부모 디렉터리)의 `.env` | 프로젝트별 키            |
...
저장소에 자체 키가 있을 때는 두 번째 방법이 여전히 유용합니다. 루트의
`.env`이 사용자 구성을 변경하지 않고도 사용자 구성보다 우선합니다.
환경에 이미 정의된 변수는 두 구성 모두보다 우선합니다.
...
`REGEN_PROVIDER`, `REGEN_MODEL` 및 `REGEN_JOB_TIMEOUT`(작업당 제한, 기본값 600초)을 사용합니다.
...
이 Provider는 API 키를 사용하지 않습니다. 공식 Codex CLI를
비대화형 모드로 제어하므로, 번역 비용은 이미 결제된 ChatGPT
구독(Plus, Pro, Business…)의 할당량에서 차감됩니다. 이는 이 용도에 대해
OpenAI가 문서화한 유일한 방법입니다. `~/.codex/auth.json`의 토큰은
...
그 다음 Python 패키지 `openai-codex-cli-bin`. 마지막 패키지는 의도적으로
`requirements.txt`에 포함되지 않습니다. 약 250MB를 차지하므로 선택적 Provider를
사용하지 않는 모든 사용자에게 부담이 되기 때문입니다.

**알아둘 점:**

- **API 키는 사용되지 않습니다.** `OPENAI_API_KEY` 및 `CODEX_API_KEY`은
  하위 프로세스의 환경에서 제거되므로, `.env`에 키가 있어도
  번역이 사용량 기반 과금으로 전환되지 않습니다.
- **세그먼트 하나는** 요금제의 5시간 창에서 **하나의 ‘로컬 메시지’**입니다.
  품질 모델(`gpt-5.6-sol`, Plus에서 5시간당 10~100개 메시지) 대신
  `--eco`(모델 `gpt-5.6-luna`, Plus에서 5시간당 250~2,000개 메시지)을 사용하세요.
- API 호출보다 **느립니다**. 직접 호출하면 몇 초 걸리는 완전한 README에
  약 45초가 걸립니다.
- **CI에서는 거부됩니다**(`CI` 또는 `GITHUB_ACTIONS`이 정의된 경우).
  구독 인증은 공유 Runner용이 아니며, OpenAI는 공개 저장소에서 이 작업 흐름을
  권장하지 않습니다. 이 경로에서는 API 키를 사용하세요.
- 환경 변수: `CODEX_BIN`(바이너리의 명시적 경로) 및
  `CODEX_TIMEOUT`(세그먼트당 초, 기본값 `600`).

### Grok 구독으로 번역(`--use_grok_cli`)

공식 **Grok Build** CLI를 사용한다는 점을 제외하면 `--use_codex`와 같은 원리입니다.
번역은 토큰 단위로 과금되지 않고 Grok 구독(SuperGrok / X Premium+)에서 차감됩니다.
...
- Grok sandbox는 최근 Linux 시스템의 많은 환경에서 적용되지 **않을 수 있습니다**.
  Ubuntu 24.04부터 AppArmor가 권한 없는 user namespace를 차단하며,
  `/run/podman`가 `0700`일 때 컨테이너 런타임 소켓의 deny-list도 실패합니다.
  그런데 적용할 수 없는 **통합** 프로필은 **조용히 비격리 상태로** 시작됩니다.
- 따라서 스크립트는 기본적으로 프로필을 요청하지 않으며, **절대 조용히 대체하지도
  않습니다**. 대신 경고를 표시합니다. 격리는 CLI의 `--deny` 규칙
  (`*` catch-all 포함)에 의존하며, 이는 측정된 유일한
  _fail-closed_ 계층입니다. 알 수 없는 규칙은 보호 기능을 알리지 않고 제거하는 대신
  시작을 거부합니다.
- OS sandbox를 **강제**하려면 `GROK_TRANSLATE_SANDBOX=read-only`를 사용하세요. 시스템이 이를
  준수할 수 없으면 시작에 실패하며, 이것이 의도된 동작입니다.

**할당량**: Grok 풀은 Chat, Imagine 및 Voice와 **주 단위로 공유**되며,
이를 조회할 수 있는 명령은 없습니다. 따라서 일괄 처리가 아무런 표시 없이
대화 사용량을 소모할 수 있습니다. 이 때문에 동시성은 2로 제한되며
`regen_translations.sh`에 경고가 표시됩니다.
...
> **Provider 플래그 6개는 상호 배타적입니다.** 이전에는 두 개를 함께 사용해도
> 조용히 허용되었으며, 먼저 검사된 항목으로 처리되었습니다. 따라서 구독 할당량으로
> 요청한 번역(`--use_codex`, `--use_grok_cli`)이 아무런 경고 없이 사용량 기반
> 과금으로 전환될 수 있었습니다. 이제 `argparse`은 이러한 조합을 거부합니다.

### 번역 노트: 위치와 형식

`--add_translation_note`을 사용하면 translator가 노트를 위, 아래 또는 두 위치에 배치하고,
일반 텍스트 형식(이전 버전과 호환) 또는 Markdown 플러그인이 사용할 수 있는
`marker` 형식으로 만들 수 있습니다.

**위치**(`--note_position`):

- `bottom`(기본값): 기존 방식처럼 파일 끝에 노트 삽입
- `top`: **YAML frontmatter 이후**에 노트 삽입(Astro Content Collections, gray-matter 등과의 호환성)
- `both`: 위와 아래에 모두 노트 삽입(LLM 호출 한 번으로 두 위치에 콘텐츠 재사용)

**형식**(`--note_format`):

- `legacy`(기본값): 굵은 문단 `**...**` — v1.8과 완전히 동일한 동작(byte-for-byte). Hugo, GitHub, GitLab 및 모든 Markdown renderer와 호환
- `marker`: 보이지 않는 Markdown 링크 참조 정의(`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`) 뒤에 굵은 blockquote를 배치합니다. GitHub/GitLab에서 기본적으로 읽을 수 있으며, Astro 측 remark 플러그인이 빌드 시 이를 활용하여 스타일이 적용된 배너를 생성할 수 있습니다(jls42.org 블로그 참조).
...
> **장문 번역 권장 사항**: `--use_gemini`(기본값 = `gemini-3.7-flash`)은
> 라틴 문자가 아닌 스크립트(PL, JA, ZH, AR, HI)의 스크립트에서 Markdown 구조를
> 충실하게 보존하며, placeholder 충실성이 중요한 `--news` 모드에서도
> 마찬가지입니다. 일본어로 번역된 이 README에서 측정한 결과, 약 6배 더 짧은 지연 시간으로
> `gemini-3.1-pro-preview`와 동일한 구조(목록 21개, 코드 블록 18개, HTML 링크 13개,
> 이미지 13개, 모든 URL 보존)를 유지했습니다. 이전 버전과의 호환성을 위해 OpenAI가
> 여전히 기본값입니다.
