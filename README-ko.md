...
이 Python 스크립트는 서식, 코드 블록, front matter 메타데이터를 유지하면서 Markdown 파일을 원본 언어에서 대상 언어로 번역합니다.

## 주요 기능

- **Multi-Provider**: 4개 API(OpenAI, Mistral, Claude, Gemini) 지원
...
이 프로젝트는 형식이 잘못되었거나 취약하거나 비밀 정보를 포함하는 코드를 커밋하는 것을 방지하기 위해 [`pre-commit`](https://pre-commit.com)을 사용합니다. 설치:
...
| `--news`                 | 뉴스 모드: EN 인용문 보호, 언어별 깃발 처리 |
...
> **장문 번역 권장사항**: `--use_gemini`(기본값 = `gemini-3.1-pro-preview` 품질, `--eco` = `gemini-3-flash-preview`)는 비라틴 문자 스크립트(PL, JA, ZH, AR, HI)에서 markdown 구조를 더 잘 보존하는 경향이 있으며, 특히 자리표시자 충실도가 중요한 `--news` 모드에서 그렇습니다. OpenAI는 하위 호환성을 위한 기본값으로 유지됩니다.
...
**이 문서는 gpt-5.4-mini 모델을 사용하여 fr 버전에서 ko 언어로 번역되었습니다. 번역 프로세스에 대한 자세한 정보는 https://github.com/jls42/ai-powered-markdown-translator 를 참조하세요.**
