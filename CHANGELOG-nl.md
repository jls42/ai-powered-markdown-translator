...
- **1.9** Silent-failure + volledige kwaliteits-tooling oplossen (2026-05-03) :
  - **Silent-failure op lange vertalingen oplossen** :
    ...
    - Whitelist `finish_reason` / `stop_reason` : `RuntimeError` genereren bij elke status buiten de whitelist (truncation, content_filter, enz.)
    ...
    - Foutdoorgifte tot aan een niet-nul exitcode: `translate_markdown_file` retourneert een getypeerde status `success` / `failure` / `skipped`, `main()` `sys.exit(1)` als minstens één bestand is mislukt (single-file en batch)
    - Empty-content guard op alle providers, sanity ratio bron/output (≥ 500 tekens, < 5% = weigering), validatie van code-placeholders (`#CODEBLOCK`/`#INLINECODE`), post-LLM-normalisatie (scheiders/links die aan een heading plakken), `BadRequestError` retry zonder `reasoning_effort`
    - Afhankelijkheid `langdetect==1.0.9` toegevoegd
  - **Pre-commit kwaliteits-tooling** ("complete EurekAI-type", 14 hooks) :
    ...
    - Pre-push : mypy (progressieve losse modus), Opengrep SAST (translate.py + scripts/), pip-audit (initiële reporting-modus), unittest discover (tests/ + scripts/tests/)
  ...
  - Documentatie : `README.md` (FR + 14 vertalingen) met badges, `CLAUDE.md` (pre-commit-workflow + gedetailleerde watch CI), 28 gegenereerde vertalingen opnieuw opgebouwd
- **1.8** `--news`-modus + modelupdates 2026 (2026-03-17, tag `v1.8`) :
  ...
  - Validatie van nieuws-placeholders vóór herstel (regressie: een LLM die de placeholder verwijderde, produceerde stilzwijgend een uitvoer zonder citaat)
...
- **1.5** Verbeteringen :
  ...
  - **Optimalisatie van vertaalprompts :**
    - De prompts voor directe vertalingen en vertaalnotities zijn uitgebreid voor meer duidelijkheid en efficiëntie, inclusief gedetailleerde instructies over het behoud van metadata en specifieke opmaakelementen.
  - **Code-refactorisatie :**
  ...
  - **Beheer van uitvoerbestanden :**
    - Omkering van model en taal in de naam van uitvoerbestanden (bijvoorbeeld `f"{base}-{args.target_lang}-{args.model}.md"`), wat organisatie en terugvinden van vertalingen vergemakkelijkt.
