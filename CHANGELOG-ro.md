...
- **1.7.3** Instrumentare calitate pre-commit (2026-04-30) :
  - Configurare `pre-commit` „type EurekAI complet” : 14 hook-uri repartizate pe două etape (pre-commit rapid + pre-push greu)
  ...
  - Pre-push : mypy (mod lax progresiv), Opengrep SAST (translate.py + scripts/), pip-audit (mod raportare inițială), unittest discover (tests/ + scripts/tests/)
  - Wrappere locale în `scripts/` care folosesc `./venv/bin/python` (sistemul nu are `python` gol în afara venv)
  ...
  - Documentație : README.md (FR) + CLAUDE.md (workflow detaliat), 28 de traduceri regenerat
...
- **1.7.2** Remediere eșec silențios la traduceri lungi (2026-04-28) :
  - Validare limbă post-traducere pe toți furnizorii (OpenAI, Mistral, Claude, Gemini) : strat determinist (extrasul sursă regăsit verbatim) + strat probabilistic (`langdetect`)
  ...
  - Segmentare orientată pe heading-uri : prioritate H2/H3 în a doua jumătate a segmentului (fiecare segment începe cu o secțiune semantică completă)
  - Propagarea erorilor până la cod de ieșire non-zero : `translate_markdown_file` returnează un statut tipat `success` / `failure` / `skipped`, `main()` `sys.exit(1)` dacă cel puțin un fișier a eșuat (single-file și batch)
...
- **1.6** Noutăți :
  ...
  - Traducere de fișier unic (`--file`) fără a parcurge un director
...
- **1.2** Remediere jurnal de modificări
