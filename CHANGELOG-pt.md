...
- **1.7** Novidades:
    ...
    - Suporte ao arquivo `.env` para carregar as chaves da API automaticamente
...
- **1.5** Melhorias:
    - **Atualização das chaves da API e dos modelos padrão:**
    ...
    - **Refatoração do código:**
        ...
        - Melhoria da segmentação de textos e gestão dos blocos de código para preservar o formato original durante a tradução.
    - **Gestão dos arquivos de saída:**
        - Inversão do modelo e da língua no nome dos arquivos de saída (por exemplo, `f"{base}-{args.target_lang}-{args.model}.md"`), facilitando a organização e a busca pelas traduções.
    - **Melhorias diversas:**
...
- **1.3** Melhorias e novas funcionalidades:
    ...
    - Inversão do modelo e da língua no nome do arquivo de saída
