...
Översättare av Markdown-filer som använder **OpenAI**, **Mistral AI**, **Claude (Anthropic)**, **Google Gemini** och **Grok (xAI)** — via API eller med kvoten från en ChatGPT-prenumeration (Codex) eller Grok, utan användningsbaserad debitering.
...
- **Flera leverantörer**: 5 API:er (OpenAI, Mistral, Claude, Gemini, Grok) + 2 prenumerationsbaserade CLI-verktyg, utan användningsbaserad debitering — Codex (ChatGPT) och Grok
...
- **Smart segmentering**: Hantering av långa texter med tokenbegränsningar per modell
...
Kommandot `aipmt` är då tillgängligt överallt. Om Python-skriptkatalogen
inte finns i din `PATH` gör `python -m aipmt` exakt samma
...
`requirements.txt` är en **fullständigt låst lock-fil**, en exakt återspegling av
den testade miljön. Gränserna som publiceras i `pyproject.toml` är
avsiktligt bredare: de påverkar inte dina andra paket.

### Kvalitetsverktyg (valfritt men rekommenderat)

Projektet använder [`pre-commit`](https://pre-commit.com) för att förhindra att fel formaterad kod, sårbar kod eller kod som innehåller en hemlighet committas. Installation:
...
Varje plats fyller bara i det som den föregående lämnar tomt.
...
Det andra alternativet är fortfarande användbart när ett arkiv har en egen nyckel: en `.env` i dess rotkatalog
har då företräde framför användarkonfigurationen utan att ändra den. Och en variabel som redan definierats i miljön har företräde framför båda:

```bash
export OPENAI_API_KEY='une-clé-le-temps-d-une-commande'
```

Om ingen nyckel hittas visar kommandot ingen anropslogg: det
listar de tre platserna med deras exakta sökvägar.

`GEMINI_API_KEY` accepteras som alternativ till `GOOGLE_API_KEY` (AI
Studio-konvention). Valfria variabler: `XAI_BASE_URL` (xAI-slutpunkt, standardvärde
...
Denna leverantör använder ingen API-nyckel: den styr det officiella Codex-CLI:t i
icke-interaktivt läge, så översättningen räknas av från den redan betalda
ChatGPT-prenumerationens kvot (Plus, Pro, Business …). Detta är den enda väg
OpenAI dokumenterar för denna användning — tokens från `~/.codex/auth.json` autentiserar
inte anrop till Platform API och läses dessutom aldrig av detta skript.
...
Binärfilen söks i denna ordning: variabeln `CODEX_BIN`, `PATH`,
och därefter Python-paketet `openai-codex-cli-bin`. Det sistnämnda ingår avsiktligt
inte i `requirements.txt`: det är ungefär 250 MB stort, vilket skulle belasta alla
användare för en valfri leverantör.

**Bra att veta:**

- **Ingen API-nyckel används.** `OPENAI_API_KEY` och `CODEX_API_KEY` tas
  bort från underprocessens miljö, vilket garanterar att en nyckel i `.env`
...
- **Långsammare** än ett API-anrop: räkna med cirka 45 s för en komplett README, jämfört med
  några sekunder direkt.
- **Nekas i CI** (`CI` eller `GITHUB_ACTIONS` definierad): prenumerationsautentisering
  är inte avsedd för en delad runner, och OpenAI avråder från detta arbetsflöde i offentliga arkiv.
  Använd en API-nyckel i detta fall.
...
**Begränsningar — läs före användning.** Denna leverantör är strukturellt **svagare**
än `--use_codex`, och det är avsiktligt:

- Codex körs i `--sandbox read-only`, en gräns som systemet kräver.
- Groks sandbox **kan inte tillämpas** på många nyare Linux-system: AppArmor blockerar
  opriviligierade user namespaces sedan Ubuntu 24.04, och deny-listan för
  containrars runtime-sockets misslyckas om `/run/podman` är `0700`. En **integrerad**
  profil som inte kan tillämpas startar däremot **utan begränsningar, i tysthet**.
- Skriptet begär därför ingen profil som standard och faller **aldrig tyst** tillbaka:
  det visar en varning. Begränsningen bygger på CLI:ts `--deny`-regler
  (inklusive catch-all-regeln `*`), det enda _fail-closed_-lagret som har
  uppmätts — en okänd regel gör att starten nekas i stället för att skyddet tas bort utan
  att det meddelas.
...
förbruka din konversationskvot utan att något indikerar det — därav en
begränsning till 2 samtidiga körningar och en varning i `regen_translations.sh`.

Övriga variabler: `GROK_BIN` (sökväg till binärfilen), `GROK_TIMEOUT` (standardvärde 900 s).
...
| `--target_dir`           | Utdatamapp för de översatta filerna                          |
| `--source_lang`          | Källspråk (standard: `fr`)                                             |
| `--target_lang`          | Målspråk (standard: `en`)                                              |
| `--model`                | Specifik modell att använda                                             |
| `--eco`                  | Använda ekonomiska modeller                                         |
| `--use_mistral`          | Använda Mistral AI API                                                |
| `--use_claude`           | Använda Claude API                                                    |
| `--use_gemini`           | Använda Gemini API                                                    |
| `--use_codex`            | Använda Codex CLI med ChatGPT-prenumerationens kvot               |
| `--use_grok`             | Använda xAI API (Grok) — kräver `XAI_API_KEY`                      |
| `--use_grok_cli`         | Använda Grok CLI med Grok-prenumerationens kvot                   |
| `--force`                | Tvinga omöversättning                                                  |
| `--keep_filename`        | Behålla det ursprungliga filnamnet                                     |
| `--news`                 | Nyhetsläge: skyddar EN-citat och hanterar flaggor per språk |
| `--add_translation_note` | Lägga till en översättningsnotis                                           |
...
> **De sex leverantörsflaggorna är ömsesidigt uteslutande.** Att kombinera två
> accepterades tidigare i tysthet och löstes till den första som testades: en
> översättning som begärdes via prenumerationskvoten (`--use_codex`, `--use_grok_cli`)
> kunde därmed i stället debiteras per användning utan någon varning.
> `argparse` avvisar numera kombinationen.

### Översättningsnotis: placeringar och format

Med `--add_translation_note` kan översättaren placera notisen högst upp, längst ned eller på båda platserna, och återge den antingen i enkelt textformat (bakåtkompatibelt) eller i formatet `marker` som kan användas av ett Markdown-plugin.

**Placering** (`--note_position`):

- `bottom` (standard): notis i slutet av filen, som historiskt.
- `top`: notis infogad **efter YAML-frontmatter** (säkerhet för Astro Content Collections, gray-matter osv.).
- `both`: notis infogad både högst upp OCH längst ned (ett enda LLM-anrop, samma innehåll återanvänds på båda platserna).
...
- `marker`: osynlig Markdown-link reference-definition (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`) följd av ett fetstilt blockquote. Läsbar direkt på GitHub/GitLab och kan användas vid bygget av ett remark-plugin på Astro-sidan för att skapa en stiliserad banner (se bloggen jls42.org).
...
> **Rekommendation för långformstexter**: `--use_gemini` (standard = `gemini-3.7-flash`) bevarar Markdown-strukturen troget för icke-latinska skript (PL, JA, ZH, AR, HI), även i `--news`-läge där det är viktigt att platshållarna bevaras. Uppmätt på denna README översatt till japanska: identisk struktur med `gemini-3.1-pro-preview` (21 listor, 18 kodblock, 13 HTML-länkar, 13 bilder, alla URL:er bevarade) med ungefär 6 gånger lägre latens. OpenAI förblir standard för bakåtkompatibilitet.
