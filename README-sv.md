...
Markdownfilsöversättare som använder **OpenAI**, **Mistral AI**, **Claude (Anthropic)** och **Google Gemini**.
...
- **Översättningsnotis**: Valfri notis läggs till i slutet av dokumentet
...
Projektet använder [`pre-commit`](https://pre-commit.com) för att förhindra att felaktigt formaterad, sårbar eller hemlig kod committas. Installation:
...
Aktiva hooks: ruff (lint+format), shellcheck (bash), prettier (markdown/yaml/json), Lizard (komplexitet), detect-secrets (API-nycklar), mypy (gradvis typning), Opengrep (SAST), pip-audit (CVE-dep), unittest. Se avsnittet `CLAUDE.md` _Quality / pre-commit_ för detaljer.
...
> **Rekommendation för långformöversättningar**: `--use_gemini` (standard = `gemini-3.1-pro-preview` kvalitet, `--eco` = `gemini-3-flash-preview`) tenderar att bättre bevara markdown-strukturen i skript med icke-latinska alfabet (PL, JA, ZH, AR, HI), särskilt i `--news`-läge där troheten mot platshållare är viktig. OpenAI förblir standard för bakåtkompatibilitet.
