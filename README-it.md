# Traduttore di Markdown basato sull'IA
...
- **Multi-Provider**: supporto di 4 API (OpenAI, Mistral, Claude, Gemini) + il CLI Codex con abbonamento ChatGPT
...
di Python non è nel tuo `PATH`, `python -m aipmt` fa esattamente la stessa
...
Il repository clonato rimane necessario per lo sviluppo: è lì che risiedono i test,
...
`requirements.txt` è un **lock completamente bloccato**, riflesso esatto dell'
ambiente testato. I limiti pubblicati in `pyproject.toml` sono
volutamente più ampi: non impongono nulla agli altri tuoi pacchetti.
...
Hook attivi: ruff (lint+format), shellcheck (bash), prettier (markdown/yaml/json), Lizard (complessità), detect-secrets (chiavi API), mypy (tipizzazione progressiva), Opengrep (SAST), pip-audit (dipendenze CVE), unittest. Consulta `CLAUDE.md` nella sezione _Quality / pre-commit_ per i dettagli.

## Configurazione

Le chiavi vengono cercate in **tre posizioni**, dalla più prioritaria alla meno prioritaria.
Ciascuna si limita a colmare ciò che la precedente lascia vuoto.

|     | Dove                                            | A cosa serve                             |
| --- | --------------------------------------------- | ------------------------------------- |
| 1   | Variabili d'ambiente                     | CI, contenitori, deroga occasionale |
...
La seconda rimane utile quando un repository ha una propria chiave: un `.env` alla sua radice
ha quindi la precedenza sulla configurazione utente, senza modificarla. E una variabile già
definita nell'ambiente ha la precedenza su entrambe:

```bash
export OPENAI_API_KEY='une-clé-le-temps-d-une-commande'
```

Se non viene trovata alcuna chiave, il comando non mostra alcuna traccia della chiamata: elenca
...
non interattiva, quindi la traduzione viene sottratta dalla quota dell'abbonamento
...
- **Non viene utilizzata alcuna chiave API.** `OPENAI_API_KEY` e `CODEX_API_KEY` vengono
  rimosse dall'ambiente del sottoprocesso, garantendo che una chiave presente in
...
la traduzione viene sottratta dall'abbonamento Grok (SuperGrok / X Premium+) anziché
essere fatturata a token.
...
**Isolamento — leggere prima dell'uso.** Questo provider è strutturalmente **più
debole** di `--use_codex` e ciò è voluto:
...
- Il sandbox di Grok **non può essere applicato** su molti sistemi Linux recenti:
  ...
  dei socket del runtime dei container fallisce se `/run/podman` è in `0700`. Ora,
  un profilo **integrato** che non può essere applicato si avvia **senza isolamento, in silenzio**.
- Lo script non richiede quindi alcun profilo per impostazione predefinita e **non ricade mai
  silenziosamente**: mostra un avvertimento. L'isolamento si basa sulle regole `--deny`
  del CLI (incluso il catch-all `*`), l'unico livello misurato
  _fail-closed_ — una regola sconosciuta fa rifiutare l'avvio anziché rimuovere la protezione
  senza comunicarlo.
- Per **richiedere** il sandbox del sistema operativo: `GROK_TRANSLATE_SANDBOX=read-only`. L'avvio
  fallirà se la macchina non può rispettarlo, comportamento voluto.
...
intaccare il tuo utilizzo conversazionale senza che nulla lo segnali — da qui una
...
> veniva accettato silenziosamente e veniva scelto il primo verificato: una
> traduzione richiesta sulla quota dell'abbonamento (`--use_codex`, `--use_grok_cli`)
> poteva quindi essere addebitata a consumo senza alcun avvertimento.
...
- `bottom` (predefinito): nota alla fine del file, come in passato.
...
- `marker`: definizione Markdown invisibile di una link reference (`[ai-translation-note-<placement>]: <> "v=1 source=… target=… model=… date=…"`) seguita da un blockquote in grassetto. Leggibile nativamente su GitHub/GitLab e utilizzabile in fase di build da un plugin remark lato Astro per produrre un banner stilizzato (vedi il blog jls42.org).
